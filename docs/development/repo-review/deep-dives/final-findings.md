...
| 3 | In progress — RWMCP-008 completed | Workspace certification and lifecycle | RWMCP-008 → RWMCP-013 → RWMCP-011 | RWMCP-008 is completed and revalidated: open, reload and commit promotion certify the complete Solution/application-to-manifest interval, commit-owned filesystem events are filtered without hiding unrelated changes, and applied commit state is revalidated before publication. Next republish reload's `Ready` advisory state, then add cancellation checks to batch target-framework evaluation without weakening the certified Workspace lease boundary. Gate: no inconsistent Ready snapshot is publishable, reload clears external advisory state and cancellation releases large-solution query leases at project boundaries. |
