...
| 2 | In progress — RWMCP-009 completed | Snapshot and selector contract foundations | RWMCP-009 → RWMCP-027 → RWMCP-012 → RWMCP-010 | RWMCP-009 is complete and revalidated. Connect nested selector validation at the Host boundary next, then centralise non-throwing path normalisation through that validated path, and finally align solution-hierarchy path identity with the resulting canonical comparer/root rules. Gate: restart replay, contradictory selectors, malformed paths, broader workspace roots and filesystem casing are covered across real adapters. |
