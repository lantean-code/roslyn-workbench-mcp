...
| `PRR-F002` | Open, release-blocking | Public planning classifies macOS as best effort, but `FileStreamWorkspaceFileLockProvider` rejects commit locking outside Windows and Linux. A macOS Host therefore cannot complete the advertised mutation workflow. | In Batch 2, either implement and validate the macOS lock and atomic-write path or explicitly exclude macOS from v1. Do not publish a best-effort support claim while transaction commit is known to be unavailable. |
