...
public sealed class ConvertBetweenRegularAndVerbatimInterpolatedStringToolIntegrationTests
{
    [Fact]
    public async Task GIVEN_ActiveTransaction_WHEN_ExecutingTool_THEN_ShouldStageMutation()
    {
        ...
        var result = await BundledCoreToolTestHarness.ExecuteMutationAsync(coordinator, "convert-between-regular-and-verbatim-interpolated-string", target, RefactoringRequestFactory.CreateLocationRequest(fixture.GetLocation("$\"C:\\\\temp\\\\{value}\""), openResult));
        ...
        result.StagedResult.Data!.Operation.Should().Be("convert-between-regular-and-verbatim-interpolated-string");
