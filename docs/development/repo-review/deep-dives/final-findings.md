...
- Async-analysis tests do not establish whether compiler diagnostic parity, general awaitable support, async local functions or async lambdas are part of the tool contract.
- Code-metrics tests verify the current formulas but do not establish or reject compatibility with Microsoft.CodeAnalysis.Metrics or Visual Studio metric values.
