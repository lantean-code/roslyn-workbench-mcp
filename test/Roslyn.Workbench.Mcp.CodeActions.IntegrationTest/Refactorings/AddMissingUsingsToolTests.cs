...
public sealed class AddMissingUsingsToolIntegrationTests
{
    [Fact]
    public async Task GIVEN_ActiveTransaction_WHEN_ExecutingTool_THEN_ShouldStageMutation()
    {
        ...
        var result = await BundledCoreToolTestHarness.ExecuteMutationAsync(coordinator, "add-missing-usings", target, new AddMissingUsingsRequest
        {
        ...
        result.StagedResult.Data!.Operation.Should().Be("add-missing-usings");
        preview.Data!.Documents.Should().Contain(static change => change.Document!.Path == "MissingUsings.cs");
    }

    [Fact]
    public async Task GIVEN_GlobalUsingPreference_WHEN_ExecutingTool_THEN_ShouldRejectUnsupportedOption()
    {
        using var fixture = await InspectionSampleFixture.CreateAsync();
        var coordinator = BundledCoreToolTestHarness.CreateBuiltInCodeActionCoordinator();
        var openResult = await coordinator.OpenAsync(new WorkspaceOpenRequest
