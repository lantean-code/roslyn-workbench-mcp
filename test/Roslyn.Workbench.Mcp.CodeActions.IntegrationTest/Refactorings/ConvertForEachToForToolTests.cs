...
public sealed class ConvertForEachToForToolIntegrationTests
{
    [Fact]
    public async Task GIVEN_ActiveTransaction_WHEN_ExecutingTool_THEN_ShouldStageMutation()
    {
        ...
        var result = await BundledCoreToolTestHarness.ExecuteMutationAsync(coordinator, "convert-foreach-to-for", target, RefactoringRequestFactory.CreateLocationRequest(fixture.GetLocation("foreach (var value in values)"), openResult));
        ...
        result.StagedResult.Data!.Operation.Should().Be("convert-foreach-to-for");
