...
| 4 | In progress | Recovery payload and persistence limits | RWMCP-016 (completed) → RWMCP-018 (completed) → RWMCP-017 | Source metadata is persisted and restored. Aggregate planning memory has been measured across representative repository sizes and accepted without a production cap; permanent scenario metrics and version-specific agent guidance retain operational control. Translate the remaining supported per-item and manifest limit failures at the commit boundary. Gate: replacement/recovery preserves supported metadata and every supported-limit rejection is structured while the transaction remains recoverable. |
