...
{
    private readonly FindUnusedSymbolsTool _target;

    public FindUnusedSymbolsToolTests()
    {
        _target = new FindUnusedSymbolsTool();
    }
    ...
    public async Task GIVEN_ResolveDocumentsHasRejection_WHEN_CallingExecuteAsync_THEN_ShouldReturnRejectionResult()
    {
        ...
        var result = await _target.ExecuteAsync(new FindUnusedSymbolsRequest(), queryContextMocks.QueryContext.Object, TestContext.Current.CancellationToken);
    ...
    public async Task GIVEN_ExcludeGeneratedIsTrue_WHEN_CallingExecuteAsync_THEN_ShouldExcludeGeneratedDocumentsFromCompilerDiagnosticsRequest()
    {
        ...
        queryContextMocks.GeneratedSourceClassifier
        ...
        var result = await _target.ExecuteAsync(new FindUnusedSymbolsRequest
        {
    ...
    [Fact]
    public async Task GIVEN_MoreUnusedSymbolsThanRequested_WHEN_CallingExecuteAsync_THEN_ShouldProjectOnlyReturnedCandidates()
    {
        using var document = RoslynTestFactory.CreateDocument("""
            class Formatter
            {
        ...
        var firstStart = source.IndexOf("firstField", StringComparison.Ordinal);
        var secondStart = source.IndexOf("secondField", StringComparison.Ordinal);
        ...
        compilerDiagnosticService
            ...
            .ReturnsAsync(
            [
                RoslynTestFactory.CreateDiagnostic("CS0169", syntaxTree!, secondStart, "secondField".Length),
                RoslynTestFactory.CreateDiagnostic("CS0169", syntaxTree!, firstStart, "firstField".Length),
            ]);
        ...
        result.Outcome.Should().Be(PluginExecutionOutcome.Succeeded);
        result.Data!.Candidates.Items.Should().ContainSingle(item => item.Symbol!.DisplayName == "firstField");
        result.Data.Candidates.HasMore.Should().BeTrue();
