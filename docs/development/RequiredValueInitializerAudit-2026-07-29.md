...
| --- | --- | --- |
| Display, identity and classification projections | `AnalyzerInfo.DisplayName`; `ApiSymbolInfo.Accessibility`; `AsyncFinding.Kind`; `AttributeInfo.Name`; `BasicBlockInfo.Kind`; `BasicBlockOperationInfo.Kind`; `CallableSignature.DisplayName`, `Kind`; `CompilationOptionsInfo.OutputKind`, `OptimizationLevel`; `ControlFlowExit.Kind`; `DependencyInfo.Kind`; `DisposableFinding.Kind`; `DocumentOptionsData.LanguageVersion`, `NullableContext`; `FlowRegionInfo.Kind`; `MetadataReferenceInfo.Display`; `OperationNode.Kind`; `OutlineNode.Name`, `Kind`; `ParameterInfo.Name`, `RefKind`; `ParseOptionsInfo.Language`, `LanguageVersion`, `DocumentationMode`; `SymbolInfoData.Accessibility`; `TypeInfo.DisplayName`, `Kind`; `UnusedSymbolCandidate.Confidence` | Required scalar projections. |
...
| Text projections | `AsyncFinding.Message`; `CodeContextData.Text`; `DisposableFinding.Message` | Required projections. The projected content may legitimately be empty, but the producer should state that value explicitly. `DuplicateCodeOccurrence.Context` and `OperationNode.Syntax` were removed when these tools changed to exact source pointers so large source text is not embedded solely for navigation. |
| Boolean and numeric result projections | `ApiSymbolInfo.IsObsolete`; `AttributeInfo.Inherited`; `BasicBlockInfo.Ordinal`, `IsReachable`; `CompilationOptionsInfo.AllowUnsafe`, `WarningLevel`; `ControlFlowAnalysisData.EntryReachable`, `ExitReachable`; `DefinitionLocation.IsMetadata`; `DocumentOutlineData.Truncated`; `DuplicateCodeGroup.StatementCount`; `FlowRegionInfo.Id`, `FirstBlockOrdinal`, `LastBlockOrdinal`; all five `ImpactSummary` counts; all five `MetricInfo` metrics; `OperationNode.HasConstantValue`, `Truncated`; `OperationTreeData.Truncated`; `ParameterInfo.IsOptional`, `HasExplicitDefaultValue`; `ReferenceLocation.IsDefinition`, `IsWrite`; `TypeHierarchyNode.Depth` | Required projections. Zero and `false` can be valid results, but an omitted projection must not silently produce them. |
