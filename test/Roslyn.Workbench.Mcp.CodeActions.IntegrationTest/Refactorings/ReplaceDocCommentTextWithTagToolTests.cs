...
public sealed class ReplaceDocCommentTextWithTagToolIntegrationTests
{
    [Fact]
    public async Task GIVEN_ActiveTransaction_WHEN_ExecutingTool_THEN_ShouldStageMutation()
    {
        ...
        var result = await BundledCoreToolTestHarness.ExecuteMutationAsync(coordinator, "replace-doc-comment-text-with-tag", target, RefactoringRequestFactory.CreateLocationRequest(fixture.GetLocation("System.IDisposable"), openResult));
        ...
        result.StagedResult.Data!.Operation.Should().Be("replace-doc-comment-text-with-tag");
