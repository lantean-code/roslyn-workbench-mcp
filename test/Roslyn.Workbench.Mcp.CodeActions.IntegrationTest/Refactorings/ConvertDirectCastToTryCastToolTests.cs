...
public sealed class ConvertDirectCastToTryCastToolIntegrationTests
{
    [Fact]
    public async Task GIVEN_ActiveTransaction_WHEN_ExecutingTool_THEN_ShouldStageMutation()
    {
        ...
        var target = new ConvertDirectCastToTryCastTool();

        openResult.Outcome.Should().Be(ToolOutcome.Succeeded);

        var result = await BundledCoreToolTestHarness.ExecuteMutationAsync(coordinator, "convert-direct-cast-to-try-cast", target, RefactoringRequestFactory.CreateLocationRequest(fixture.GetLocation("(object)1"), openResult));
        ...
        result.StagedResult.Data!.Operation.Should().Be("convert-direct-cast-to-try-cast");
