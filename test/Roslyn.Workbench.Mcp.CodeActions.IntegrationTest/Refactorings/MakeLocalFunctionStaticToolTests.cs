...
public sealed class MakeLocalFunctionStaticToolIntegrationTests
{
    [Fact]
    public async Task GIVEN_ActiveTransaction_WHEN_ExecutingTool_THEN_ShouldStageMutation()
    {
        ...
        var target = new MakeLocalFunctionStaticTool();

        openResult.Outcome.Should().Be(ToolOutcome.Succeeded);

        var result = await BundledCoreToolTestHarness.ExecuteMutationAsync(coordinator, "make-local-function-static", target, RefactoringRequestFactory.CreateLocationRequest(fixture.GetLocation("Local", 0), openResult));
        ...
        result.StagedResult.Data!.Operation.Should().Be("make-local-function-static");
