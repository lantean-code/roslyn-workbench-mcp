...
- [ ] `ALPHA-006` Configure issue intake and triage — **In progress**; forms, labels and triage guidance are approved and committed, and the 37 managed labels are synchronised on GitHub. Form dry runs remain outstanding.
  - [x] Local forms, label manifest, synchronisation tools and triage guidance
  - [x] Live label synchronisation
  - [ ] Rendered-form dry runs
...
- [ ] `ALPHA-008` Harden GitHub repository controls and automation — **In progress**; local implementation is reviewed, Actions defaults and Dependabot protections are configured, and hosted unit/contract CI passes. Broader hosted validation is in progress; rulesets and secret scanning remain unavailable for the private repository.
...
Replicate qbtmud's MkDocs and Material documentation approach: keep pinned documentation dependencies, explicit navigation, search, system/light/dark themes and copyable code blocks. Following the revised 2026-09-02 decision, run strict site builds when relevant changes reach `develop` and during release builds, not on pull requests or a separate documentation dispatch. Deploy to `https://lantean-code.github.io/roslyn-workbench-mcp/` through the approved development and release publication paths. Pin reusable GitHub Actions to reviewed commit SHAs under the repository's release-security policy.
...
- Generate the reference as part of every ordinary documentation pipeline from the Host compiled for that source: `develop` deploys the complete site to `/dev/`, and approved releases deploy it to `/{source-tag}/` and update latest. Pull requests do not run the documentation workflow. Trigger the development site build for relevant runtime code, contract, schema, example, template and authored-site changes, excluding development records and analyser-only changes; retain the analyser shared source compiled into the runtime as an input. Do not commit generated Markdown or JSON to `develop`; retain authored inputs and tests there and publish generated output only through the validated site artifact and `gh-pages`.
...
- Use two levels of validation guidance. Changes entering `develop`, `release/*` or `hotfix/*` should restore and build the solution, run unit and contract tests, verify the minimum test count, and apply the repository's normal compiler and analyser configuration. Run strict documentation validation for relevant changes after they reach `develop` and during release builds, not on pull requests. Do not make integration, acceptance, compatibility-audit, external-scenario or `latest-all` validation part of routine entry into `develop`.
...
- Retain ordinary successful test results for 14 days, failed test and acceptance diagnostics for 30 days, and periodic Code Action audit evidence for 30 days. Documentation PR previews are no longer produced under the revised 2026-09-02 policy. Attach useful release artifacts and evidence to the GitHub Release where appropriate, and otherwise use GitHub's repository default for workflow logs without adding separate archival infrastructure.
...
**Implementation evidence:** Routine CI runs the minimum-count unit and contract suite, while `validation/full`, pull requests into `main` and manual dispatch add the component-integration and Linux and Windows published-Host acceptance jobs. Ordinary label events now skip every job; concurrency is scoped to eligible jobs and individual matrix entries so classification labels cannot cancel legitimate validation. The Code Action audit remains isolated to its weekly schedule and manual dispatch, with release preparation running the same audit explicitly. All external actions are pinned to reviewed full commit SHAs and ordinary jobs are read-only. Documentation runs only for relevant changes reaching `develop` and through the release workflow; PR previews are removed. The exact qbtmud pull-request template, managed labels and grouped Dependabot configuration are committed, and the live labels, Actions defaults, Dependabot protections and publication environments have been applied. The external setup checklist records hosted results and the remaining private-plan restrictions and publication setup.
...
**Remediation validation:** Workflow lint, focused workflow-structure checks and Bash syntax checks pass. The corrected acceptance harness passes scoped formatting and a Release build without warnings or errors; `latest-all` reports only existing warnings in unchanged fixture code. The initial complete Linux acceptance wrapper passed all 70 tests with both the default `0.1.0-alpha.1` fixture publish and the installed `0.1.0-alpha.999` package, with the latter also using explicit temporary-directory routing. Subsequent hosted full validation passed Windows acceptance, while Linux exposed two test-only CRLF assumptions that the local checkout had masked. The current corrections exercise both LF and CRLF explicitly in the rename/commit and linked-document selector scenarios. Current validation evidence is recorded in the external setup checklist; no package or versioned documentation was published externally during remediation.
