...
public sealed class NameTupleElementToolIntegrationTests
{
    [Fact]
    public async Task GIVEN_ActiveTransaction_WHEN_ExecutingTool_THEN_ShouldStageMutation()
    {
        ...
        var result = await BundledCoreToolTestHarness.ExecuteMutationAsync(coordinator, "name-tuple-element", target, RefactoringRequestFactory.CreateLocationRequest(fixture.GetCursor("return (1 + 1, 2);", 0, "return (".Length), openResult));
        ...
        result.StagedResult.Data!.Operation.Should().Be("name-tuple-element");
