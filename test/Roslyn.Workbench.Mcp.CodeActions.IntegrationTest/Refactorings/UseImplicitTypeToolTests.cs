...
public sealed class UseImplicitTypeToolIntegrationTests
{
    [Fact]
    public async Task GIVEN_ActiveTransaction_WHEN_ExecutingTool_THEN_ShouldStageMutation()
    {
        ...
        var result = await BundledCoreToolTestHarness.ExecuteMutationAsync(coordinator, "use-implicit-type", target, RefactoringRequestFactory.CreateLocationRequest(fixture.GetCursor("StringBuilder implicitBuilder", 0, "StringBuilder".Length), openResult));
        ...
        result.StagedResult.Data!.Operation.Should().Be("use-implicit-type");
