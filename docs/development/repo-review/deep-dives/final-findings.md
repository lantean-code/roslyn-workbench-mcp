...
| 3 | In progress — RWMCP-008 and RWMCP-013 completed | Workspace certification and lifecycle | RWMCP-008 → RWMCP-013 → RWMCP-011 | RWMCP-008 and RWMCP-013 are completed and revalidated: open, reload and commit promotion certify the complete Solution/application-to-manifest interval; commit-owned filesystem events are filtered without hiding unrelated changes; applied commit state is revalidated; and successful reload republishes `Ready` with transaction and commit metadata cleared. Next add cancellation checks to batch target-framework evaluation without weakening the certified Workspace lease boundary. Gate: no inconsistent Ready snapshot is publishable, reload clears external advisory state and cancellation releases large-solution query leases at project boundaries. |
