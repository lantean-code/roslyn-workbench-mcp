...
public sealed class InvertLogicalToolIntegrationTests
{
    [Fact]
    public async Task GIVEN_ActiveTransaction_WHEN_ExecutingTool_THEN_ShouldStageMutation()
    {
        ...
        var result = await BundledCoreToolTestHarness.ExecuteMutationAsync(coordinator, "invert-logical", target, RefactoringRequestFactory.CreateLocationRequest(fixture.GetLocation("&&"), openResult));
        ...
        result.StagedResult.Data!.Operation.Should().Be("invert-logical");
