...
public sealed class ReplaceConditionalWithStatementsToolIntegrationTests
{
    [Fact]
    public async Task GIVEN_ActiveTransaction_WHEN_ExecutingTool_THEN_ShouldStageMutation()
    {
        ...
        var result = await BundledCoreToolTestHarness.ExecuteMutationAsync(coordinator, "replace-conditional-with-statements", target, RefactoringRequestFactory.CreateLocationRequest(fixture.GetLocation("value = enabled ? 1 : 2;"), openResult));
        ...
        result.StagedResult.Data!.Operation.Should().Be("replace-conditional-with-statements");
