...
| 13 | In progress | Scenario-runner evidence integrity | RWMCP-038 → RWMCP-034 → RWMCP-036 → RWMCP-035 → RWMCP-037 | RWMCP-038 is completed and revalidated: one runner owns an entire repository cache from preflight through final validation, while distinct `--cache` paths retain deliberate parallelism. Next validate preparation effects while holding that ownership. Then persist partial evidence before repairing trace and cancellation commands so later failures remain diagnosable. Finally correct EventPipe commit tracing and enforce cancellation outcomes. |
