...
public sealed class ReverseForStatementToolIntegrationTests
{
    [Fact]
    public async Task GIVEN_ActiveTransaction_WHEN_ExecutingTool_THEN_ShouldStageMutation()
    {
        ...
        var result = await BundledCoreToolTestHarness.ExecuteMutationAsync(coordinator, "reverse-for-statement", target, RefactoringRequestFactory.CreateLocationRequest(fixture.GetLocation("for (var i = 0; i < values.Length; i++)", 0), openResult));
        ...
        result.StagedResult.Data!.Operation.Should().Be("reverse-for-statement");
