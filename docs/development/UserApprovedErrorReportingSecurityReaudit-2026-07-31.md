...
| `ER-SEC003` | Resolved | External preparation uses a provider-neutral versioned projection built from an allow-list. It excludes exception messages, paths, source, user-authored identifiers, Workspace identity, repository and project identity, user and machine identity, stable installation identity, environment and command-line data, credentials, prompts, conversation content and raw logs. Unknown and external implementation details are generalised before dispatch. |
...
| `ER-SEC005` | Resolved | Preparation performs no network operation and stores the immutable allow-listed external report in a capacity-isolated absolute-expiry store. A representative provider preview, destination and SHA-256 preview digest are returned before submission. `submit-error-report` accepts only the opaque handle and passes the stored report to the dispatcher. The Sentry dispatcher maps it to a strongly typed SDK event with a stable message template, formatted message and privacy-safe grouping fingerprint; SDK-assigned event and transport metadata are not represented as byte-for-byte reviewed content. |
| `ER-SEC006` | Resolved | Consent fails closed. `never` and `always` can be established only by exact command-line input; invalid input becomes `never`, and the ambient consent environment variable is ignored with a warning. Prompt mode requires advertised MCP form elicitation. Decline, cancellation, Workspace approval, session approval and session suppression have bounded process-local effects, and no choice permits unsolicited submission. Provider background delivery begins only after explicit SDK acceptance. |
