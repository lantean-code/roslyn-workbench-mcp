...
public sealed class AnalyzeDataFlowToolTests
{
    ...
    public async Task GIVEN_LocationIsNull_WHEN_CallingExecuteAsync_THEN_ShouldReturnInvalidRequest()
    {
        ...
        var result = await target.ExecuteAsync(new AnalyzeDataFlowRequest(), context, CancellationToken.None);
    ...
    [Fact]
    public async Task GIVEN_SelectedNodeHasNoStatementAncestor_WHEN_CallingExecuteAsync_THEN_ShouldReturnInvalidRequest()
    {
    ...
    [Fact]
    public async Task GIVEN_ExecutableStatement_WHEN_CallingExecuteAsync_THEN_ShouldReturnDataFlowAnalysis()
    {
        using var workspace = MiniWorkspaceFactory.CreateCSharp("""
            namespace Sample;
        ...
        var result = await target.ExecuteAsync(new AnalyzeDataFlowRequest
        {
            Location = workspace.GetLocationSelector("var trimmed = value.Trim();"),
        }, context, CancellationToken.None);

        result.Outcome.Should().Be(ToolOutcome.Succeeded);
        result.Data!.VariablesDeclared.Should().Contain(symbol => symbol.DisplayName.Contains("trimmed", StringComparison.Ordinal));
        result.Data.ReadInside.Should().Contain(symbol => symbol.DisplayName.Contains("value", StringComparison.Ordinal));
    }
}
...
public sealed class FindCalleesToolTests
{
    ...
    public async Task GIVEN_SymbolAndLocationAreBothNull_WHEN_CallingExecuteAsync_THEN_ShouldReturnInvalidRequest()
    {
        ...
        var result = await target.ExecuteAsync(new FindCalleesRequest(), context, CancellationToken.None);
    ...
    [Fact]
    public async Task GIVEN_SelectedSymbolHasNoExecutableBody_WHEN_CallingExecuteAsync_THEN_ShouldReturnInvalidRequest()
    {
        using var workspace = MiniWorkspaceFactory.CreateCSharp("""
            namespace Sample;
    ...
    [Fact]
    public async Task GIVEN_SelectedLocationDoesNotResolveToExecutableCode_WHEN_CallingExecuteAsync_THEN_ShouldReturnInvalidRequest()
    {
        using var workspace = MiniWorkspaceFactory.CreateCSharp("""
            namespace Sample;
    ...
    [Fact]
    public async Task GIVEN_ExecutableSymbolWithIndirectTraversal_WHEN_CallingExecuteAsync_THEN_ShouldReturnDirectAndIndirectCallees()
    {
        using var workspace = MiniWorkspaceFactory.CreateCSharp("""
            namespace Sample;
        ...
        var result = await target.ExecuteAsync(new FindCalleesRequest
        {
            Symbol = new SymbolSelector
            {
                DocumentationCommentId = "M:Sample.Formatter.A",
            },
            IncludeIndirect = true,
        }, context, CancellationToken.None);

        result.Outcome.Should().Be(ToolOutcome.Succeeded);
        result.Data!.Callees.Items.Should().Contain(symbol => symbol.DisplayName.Contains("B()", StringComparison.Ordinal));
        result.Data.Callees.Items.Should().Contain(symbol => symbol.DisplayName.Contains("C()", StringComparison.Ordinal));
    }
}
