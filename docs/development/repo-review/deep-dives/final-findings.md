...
| 9 | In progress | Code Action reference semantics | RWMCP-025 → RWMCP-029 | Make reference-cache admission failure truthful before changing retry metadata, so clients receive a stable capacity outcome even before they honour the corrected non-idempotent annotation. Gate: saturated/oversized recipes cannot produce false empty success, and repeated reference-producing queries are not advertised as idempotent. RWMCP-025 is complete; RWMCP-029 remains. |
