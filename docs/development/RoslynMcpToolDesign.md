...
- The server builds a `WorkspaceInputManifest` at load/reload. It includes source documents; solution, project and imported build files; editorconfig, additional and analyzer-config documents; resolved metadata/analyzer paths; and package assets that affect the loaded workspace. Watchers set a cheap dirty hint, but are not authoritative. Before semantic work, the coordinator validates the relevant manifest entries using metadata and content fingerprints where metadata changed. A document semantic operation validates its project and compilation dependency closure; a solution-wide operation validates the full manifest. A mismatch transitions `Ready` to `WorkspaceOutOfDate` or `TransactionActive` to `TransactionConflicted`.
