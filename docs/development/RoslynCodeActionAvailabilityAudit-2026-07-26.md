...
The classifications below therefore cover the complete composed runtime inventory. Providers remain hidden until their stated prerequisite and promotion evidence are complete.
...
The composed runtime contains 113 visible providers and 137 hidden providers. The hidden providers now divide as follows:

| Outcome | Count | Release interpretation |
| --- | ---: | --- |
| Replay candidate | 13 | The remaining compiler-backed code fixes require compatibility validation rather than a new transformation. |
...
Forty-seven providers consume compiler diagnostics already available from `Compilation.GetDiagnostics`. Thirty-four providers have passed deterministic action selection and transaction-staging validation and are published through thirty-three dedicated tools. Thirteen providers remain tracked as `PendingReplayValidation` only by the development audit. Compiler diagnostic availability establishes reachability but does not by itself prove deterministic action selection or staging.
...
Validation completed with 173 runtime-provider audit tests and 50 published-host acceptance tests. The acceptance suite includes a catalogue-locked matrix for all 82 dedicated Code Action tools and 101 independent execution paths across the default, C# 4 and C# 7.3 controlled workspaces. It verifies that every tool is published, then independently stages, previews and rolls back a successful invocation while proving that changed files are restored and newly created files are absent after rollback. A separate published-host workflow lists two otherwise-identical `CS8603` fixes from one broad range, verifies their distinct resolved locations and stages each correct mutation independently using only its opaque GUID. The 2026-07-27 catalogue measurement contains 138 tools and a 227,417-byte UTF-8 `tools/list` result; its 87 Code Action tools account for 143,590 bytes, the 82 dedicated Code Action tools account for 137,807 bytes and the 31 fixed compiler Code Fix tools account for 51,947 bytes. The complete GuardClauses scenario suite also passed against the expanded catalogue, including Code Action discovery and mutation staging; latest evidence: `artifacts/performance/results/20260727-183403-guardclauses-e06604204ba54ef3ab789e2f75c26c43`.
...
`CandidateCompatibilityCases` permanently tracks the thirteen compiler-backed code-fix candidates that are not yet published. Thirty-four providers live in the supported compatibility suite and are published through thirty-three dedicated tools. Each advertised diagnostic route passes the real pinned MEF composition, expected compiler diagnostic, deterministic action selection, supported `ApplyChangesOperation`, expected source mutation, dedicated handler, transaction staging and preview path.

The first remaining-candidate batch promoted twelve additional providers across nineteen advertised diagnostics: asynchronous `foreach` and `using` statements, same-variable disambiguation, documentation-node addition and removal, captured-variable parameterisation for static local functions, static-member repair, method and anonymous-function async conversion, ref-struct repair, abstract and partial type repair, sealed-base repair and the legacy interpolated-verbatim prefix order. Eleven providers use the fixed-provider replay shape. `make-method-asynchronous` instead requires an explicit `ReturnTask` or `StayVoid` strategy because `CS4033` intentionally produces both `Make method async` and `Make method async (stay void)`. The two `CS4033` actions are independently covered, producing twenty compatibility and published-host execution cases in total.

Promotion validation is diagnostic-specific rather than provider-wide. Every Code Action that supports more than one diagnostic ID must be assessed for whether those diagnostics can coexist within one requested location, whether each route selects the same target and whether each route produces an equivalent mutation. The compatibility suite must contain a replay case for every diagnostic ID the action advertises; validating one provider action does not establish its other diagnostic routes. Published-host acceptance coverage must also invoke every advertised ID independently and prove successful staging, preview and rollback through the MCP contract. When the routes can produce distinct candidates, a dedicated location-targeted tool must require the caller to select one supported ID and must reject unsupported IDs before discovery. A tool may omit explicit diagnostic selection, or intentionally aggregate multiple IDs, only when its per-ID compatibility and acceptance evidence prove that behaviour is deterministic and unambiguous. A precise diagnostic location remains required to distinguish multiple occurrences of the selected ID. This rule applies retrospectively to current multi-diagnostic actions and prospectively to every provider considered for promotion.

The diagnostic-specific check now covers all 27 advertised IDs across the current multi-diagnostic tools. Each route succeeds through published-host staging, preview and rollback. The diagnostic routes can target different declarations, but the existing `LocationSelector` provides the necessary disambiguation where the provider offers one action shape; adding a separate diagnostic-ID request property would be redundant and would still not distinguish multiple occurrences of the same ID. `make-method-asynchronous` uses its explicit strategy to distinguish the two legitimate `CS4033` actions. Dedicated location-targeted tools continue to reject broad locations containing distinct candidates as ambiguous. Broad `list-code-actions` queries instead publish one precise resolved location and opaque replay reference per candidate, allowing the caller to choose and stage the intended occurrence by GUID.
...
P0 and P1 are complete, and P2a has promoted 40 of the original 53 replay candidates through dedicated tools. Thirteen compiler-backed candidates still require fixtures. Further release scope can be selected from this group without first building general IDE diagnostic support; the 94 IDE-diagnostic providers form a separate infrastructure decision rather than an accidental pre-release commitment. Public-API and custom semantic implementations can remain future functionality without weakening the current supported surface.

No unavailable family should be promoted solely from source inspection. A controlled fixture must prove that Roslyn offers the action, Workbench selects it deterministically, staging succeeds and the public surface exposes it intentionally. For an action that advertises multiple diagnostic IDs, compatibility and published-host acceptance evidence are required separately for every advertised ID and must determine whether explicit diagnostic selection is required.
