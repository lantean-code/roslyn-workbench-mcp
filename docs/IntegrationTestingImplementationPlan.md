...
- [ ] Catalogue every file and option currently generated by `InspectionSampleFixture` and `TestWorkspaceFixture`.
- [ ] Group generated inputs into the smallest set of stable fixture templates.
- [ ] Add readable checked-in `.sln`, `.slnx`, `.csproj`, source, configuration and imported-build files under `test/TestAssets`.
- [ ] Preserve intentionally malformed inputs as explicit assets with descriptive directory names.
- [ ] Add a small asset materialiser that copies a template into a unique temporary scenario root.
- [ ] Allow token substitution only for values that must vary, such as absolute paths or unique assembly names.
- [ ] Keep source code out of large C# string literals.
- [ ] Ensure materialisation excludes `bin`, `obj`, `.vs` and prior recovery state.
- [ ] Preserve exact encodings and binary fixtures where the scenario asserts bytes.
- [ ] Migrate one read-only Workspace scenario as the pattern check.
- [ ] Migrate one mutable transaction or Code Action scenario as the isolation pattern check.
- [ ] After both patterns are approved, migrate remaining fixture consumers in batches by project.
- [ ] Delete superseded generation code only after all consumers have moved.
...
- [ ] Mutation, external-change, recovery and locking scenarios always receive isolated copies.
- [ ] No shared fixture may be mutated by cleanup, generated-code output or Workspace operations.
- [ ] Every materialised fixture exposes its workspace root, state root and asynchronous cleanup through one owner object.

### Verification

- [ ] Asset files are understandable without reading fixture-generation code.
- [ ] Existing retained scenarios pass against assets.
- [ ] Byte/encoding-sensitive tests still compare exact bytes.
- [ ] Running the same project repeatedly produces no modified checked-in asset.
- [ ] No asset contains build output or machine-specific paths.

### Exit criteria

- [ ] `InspectionSampleFixture` no longer embeds the sample project as large C# strings.
- [ ] All active Workspace templates are checked in and copied through one narrow materialiser.
