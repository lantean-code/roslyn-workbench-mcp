...
Transaction preview and review provenance identifies every active staged operation by revision, operation name and summary. Revisions created by `stage-code-action` additionally contain response-local references to concise Code Action provider attribution: runtime type, assembly simple name and assembly version, plus the Fix All provider when one assembled the action. The response-level `providers` object deduplicates those identities and is resolved by key rather than entry order. Undo excludes inactive revisions and redo restores their immutable provenance. Other mutation tools do not manufacture Code Action attribution, and provenance does not affect the exact persistence-set digest used by approval receipts.

Actual Roslyn source-generated documents and regular source under `obj` are not addressable mutation targets. For checked-in source that appears generated from its filename or opening auto-generated header, the default `warn` policy allows staging but returns a bounded `GeneratedSourceMutation` warning. Preview recalculates that warning across the complete active transaction. Approval-required review classifies each persistence entry as `project-source` or `generated-looking-source`; this classification is included in the receipt digest. An operator can select `deny` and configure narrow Workspace-relative wildcard exceptions for intentionally maintained generated source. Exceptions never bypass containment, external-document, source-kind or intermediate-output denials. This deterministic policy does not request separate elicitation.
