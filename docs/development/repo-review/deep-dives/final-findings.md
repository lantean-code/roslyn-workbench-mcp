...
| 13 | In progress | Scenario-runner evidence integrity | RWMCP-038 → RWMCP-034 → RWMCP-036 → RWMCP-035 → RWMCP-037 | RWMCP-038 and RWMCP-034 are completed and revalidated: one runner owns an entire repository cache, preparation restores only EF Core's three declared sentinel-file effects, and both pre/post certification reject any remaining tracked drift with exact paths. Next persist partial evidence before repairing trace and cancellation commands so later failures remain diagnosable. Finally correct EventPipe commit tracing and enforce cancellation outcomes. |
