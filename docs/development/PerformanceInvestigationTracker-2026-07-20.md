...
The reference traces in investigation 6 established a repeatable approximately 0.8-second Roslyn discovery cost after compilation was warm. The Workspace-owned `ReferenceDiscoveryService` stores only complete successful reference-discovery results in the DI-managed bounded query cache and is exposed to plugins through `IToolExecutionServices`; `FindReferencesTool` no longer owns Roslyn discovery or its cache-entry construction. Options configure the underlying cache's 50,000-unit size limit and ten-minute sliding expiration. Lifecycle code receives a distinct `WorkspaceQueryCache` invalidation object, while private shared state coordinates per-workspace invalidation without making the tool-facing services invalidatable. Each reference entry is sized by its referenced-symbol and reference-location counts.
