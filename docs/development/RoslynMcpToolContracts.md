...
The official C# MCP SDK supports generated input schemas, optional structured output schemas, titles and these behavioural annotations. Plugin tools are registered as server-owned `PluginMcpServerTool` instances, one per validated `RegisteredTool`, rather than by scanning plugin methods for MCP attributes. The adapter publishes the schema and metadata held by `RegisteredTool`, deserializes the full argument object into the named request record, and returns a structured `CallToolResult` carrying the published response family for that tool. See the [C# SDK tool documentation](https://csharp.sdk.modelcontextprotocol.io/concepts/tools/tools.html) and [McpServerTool API](https://csharp.sdk.modelcontextprotocol.io/api/ModelContextProtocol.Server.McpServerTool.html).

The adapter may attach plugin identity and contract version as internal metadata for diagnostics, but requirements that an agent must act upon are always stated in the standard tool description. No client-specific metadata extension is required for normal operation. Tool descriptions may include a short result hint when it materially improves discovery, but they do not restate full response DTOs or JSON Schema fragments.

External plugins are composed only from immediate package directories beneath configured `plugin-directory` search roots. Exactly one assembly in each package carries `RoslynPluginAttribute`; the attribute and the entry assembly's informational SemVer are the package identity contract, with no JSON manifest. MEF configuration is materialised into the same `RegisteredTool` model before any MCP adapter is registered, so external composition does not alter the wire contracts described here.

For implementation, each input type is named by converting the tool name to PascalCase and appending `Request`: for example, `find-references` uses `FindReferencesRequest`. Tool handlers still execute against typed C# request and response contracts, but the MCP adapter shapes the runtime JSON into the published response family for that tool rather than exposing one universal success envelope.

## Published Response Families

Every tool shares the same minimal machine-readable failure and continuation base:
...
`BoundedCollection<TItem>` has the shape `{ items: TItem[], hasMore: boolean, totalCount?: int }`. `totalCount` describes the complete post-filter result count before the response bound when that count is already known or cheaply available. It is omitted when calculating it would require continuing otherwise bounded Roslyn analysis.
...
The server resolves every selector against the effective solution: the loaded baseline outside a transaction, or the current staged solution during one. All positions and spans are zero-based UTF-16 values, directly aligned with Roslyn `TextSpan`.
...
| `TextSpanRange` | `start: int`, `length: int`. |
| `TextSpanSelector` | `document: DocumentSelector`, `range: TextSpanRange`. |
...
| `CanonicalLocationSelector` | `span: TextSpanSelector`. A span-only result projection whose JSON is directly usable as `LocationSelector` input without publishing copied-selection text. |
...
| `ResolvedLocation` | `{ document?: DocumentReference, span?: { start: int, length: int }, line: int, column: int, snapshot: SnapshotPrecondition, selector?: CanonicalLocationSelector }`. The nested snapshot is the exact identity against which the location was resolved. Host-produced source locations include the canonical selector when the document and span can be projected safely. |
...
`M` is marked destructive because a staged source replacement or deletion is a destructive update to the transaction environment even though it is reversible. `C` is destructive. `transaction-history` is an `S` exception and is also destructive because an undo followed by a new mutation drops the redo branch.
...
Code Action IDs are opaque GUID references to revalidation recipes in a bounded, process-local cache, not handles to cached provider objects. Expiry is controlled by the startup `CodeActionReferenceLifetime`, initially five minutes, and use does not extend the absolute expiry. Callers must echo the reference unchanged. Successful transaction staging consumes it, while rejected or failed staging retains it for retry. An unknown, expired, evicted or snapshot-invalid reference is rejected with recovery guidance to resolve the target again. `list-code-actions` omits excluded leaves. If its replay-reference cache cannot retain every returned action, the request is rejected with `ActionReferenceCapacityExceeded`; references created earlier in that failed request are removed, and the caller can retry after existing references expire, lower the request limit, or increase `--code-action-reference-cache-size-limit`.
...
| `workspace-open` | New server | S | **Open Workspace**. Loads an additional `.sln`, `.slnx` or `.csproj` as a writable workspace session. Open only a fully trusted workspace because loading evaluates MSBuild project logic and later diagnostic or Code Action operations can execute project analyzers with Host permissions. Unsupported languages, projects without usable paths and non-SDK-style projects are skipped with diagnostics; at least one supported SDK-style C# project must remain. Workspace-root containment follows the mounted filesystem's case semantics, including case-insensitive Windows drives accessed through WSL, while native Linux filesystems remain case-sensitive. Evaluated source, additional and analyzer-config documents outside `workspaceRoot` are admitted automatically as queryable read-only inputs, certified against disk and individually polled for changes; `msBuildProperties` affect evaluation but do not grant filesystem permissions. Mutations remain constrained to `workspaceRoot`. When the server runs under WSL and the loaded path is on a mounted Windows filesystem, the successful response includes an advisory performance warning recommending WSL-native storage or running the server directly on Windows. If instance status reports that the workspace is or may be in use elsewhere, use it only for necessary queries, expect results to become stale, and coordinate mutation ownership before starting a transaction. | `path: string` - absolute solution or project path; `alias?: string` - optional caller-friendly label; `workspaceRoot?: string` - optional absolute repository or coordination root; `msBuildProperties?: { artifactsPath?, configuration?, platform?, targetFramework?, runtimeIdentifier? }` - closed allowlist of per-workspace MSBuild global properties. | `WorkspaceOpenData { workspace: WorkspaceIdentity, projectCount, documentCount, loadDiagnostics: DiagnosticInfo[] }` |
...
| `workspace-status` | Replaces `diagnose` | Q | **Workspace Status**. Returns one selected loaded workspace's lifecycle, transaction and advisory cross-instance state. When an external input change has been detected, the response retains the first detection source, change kind and affected path so agents can distinguish watcher signals from metadata validation. Treat a workspace that is or may be in use elsewhere as query-only, use it only when necessary, and expect results to become stale. | `workspace?: WorkspaceSelector`, `detail?: Minimal \| Standard \| Full = Standard`. | `WorkspaceStatusData { state: Ready \| TransactionActive \| TransactionConflicted \| WorkspaceOutOfDate, workspace: WorkspaceIdentity, projectCount: int, documentCount: int, loadDiagnostics?: DiagnosticInfo[], transaction?: TransactionInfo, reloadRequired: boolean, externalChange?: WorkspaceExternalChangeData { detectionSource, kind, path?, previousPath? }, instances: WorkspaceInstanceInfo[] }` |
| `workspace-reload` | New server | S | **Reload Workspace**. Reloads one selected workspace that is out of date with its original effective MSBuild properties. Unavailable while that workspace owns an active transaction. | `workspace?: WorkspaceSelector`. | `WorkspaceReloadData { workspace: WorkspaceIdentity, projectCount, documentCount, loadDiagnostics: DiagnosticInfo[] }` |
| `get-solution-structure` | New plugin | Q | **Get Solution Structure**. Returns solution folders, projects, target frameworks and direct project relationships. | `includeDocuments?: boolean = false`, `foldersLimit?: int = 200`, `projectsLimit?: int = 100`, `documentsPerProjectLimit?: int = 200`, `projectReferencesPerProjectLimit?: int = 50`. | `SolutionStructureData { solutionPath, folders: BoundedCollection<SolutionFolderInfo>, projects: BoundedCollection<ProjectStructureInfo { projectReferences: BoundedCollection<ProjectReferenceInfo>, documents?: BoundedCollection<DocumentReference> }> }` |
...
| Tool | Source | Behaviour | Title and description | Input parameters | Success payload shape |
| --- | --- | --- | --- | --- | --- |
| `get-document-outline` | Existing | Q | **Get Document Outline**. Returns the bounded semantic namespace, type and member hierarchy of one document. | `document: DocumentSelector`, `includeMembers?: boolean = true`, `maxDepth?: int = 16 (maximum 24)`, `nodesLimit?: int = 200 (maximum 2000)`. | `DocumentOutlineData { document: DocumentReference, root?: OutlineNode, truncated: boolean }` |
| `get-code-context` | New | Q | **Get Code Context**. Returns a focused code window and optional semantic context branches. | `location: LocationSelector`, `beforeLines?: int = 10` (maximum `100`), `afterLines?: int = 10` (maximum `100`), `includeDiagnostics?: boolean = false`, `includeEnclosingSymbols?: boolean = false`, `enclosingSymbolsLimit?: int = 16`, `diagnosticsLimit?: int = 50`. | `CodeContextData { location: ResolvedLocation, text: string, enclosingSymbols: BoundedCollection<SymbolReference>, diagnostics: BoundedCollection<DiagnosticInfo> }` |
| `search-symbols` | Existing | Q | **Search Symbols**. Searches declarations by name, metadata name and optional semantic filters. | `query?: string`, `metadataName?: string`, `scope?: ScopeSelector`, `kinds?: string[]`, `accessibilities?: string[]`, `namespace?: string`, `symbolsLimit?: int`. At least one of `query` or `metadataName` is required. The selected solution, project, projects or exact document scope restricts eligible declarations before ordering and bounding. | `SymbolSearchData { symbols: BoundedCollection<SymbolReference> }` |
| `resolve-symbol` | New | Q | **Resolve Symbol**. Resolves the symbol at a location or selection and returns its canonical selector. | `location: LocationSelector`, `declarationsLimit?: int = 32`. | `ResolveSymbolData { symbol: SymbolReference, selector: SymbolSelector, declarations: BoundedCollection<ResolvedLocation> }` |
| `get-symbol-info` | Existing | Q | **Get Symbol Info**. Returns detailed metadata for a resolved symbol. | `symbol: SymbolSelector`, `includeDocumentation?: boolean = false`, `parametersLimit?: int = 64`, `declarationsLimit?: int = 32`. | `SymbolInfoData { symbol: SymbolReference, accessibility, modifiers: string[], type?: TypeInfo, parameters?: BoundedCollection<ParameterInfo>, returnType?: TypeInfo, documentation?: string, declarations: BoundedCollection<ResolvedLocation> }` |
...
| `go-to-definition` | Existing | Q | **Go To Definition**. Returns source or metadata definitions for a symbol. | `symbol: SymbolSelector`, `definitionsLimit?: int = 32`. | `DefinitionData { symbol: SymbolReference, definitions: BoundedCollection<DefinitionLocation> }` |
...
| `find-callers` | Existing | Q | **Find Callers**. Returns direct source call sites and containing symbols. | `symbol: SymbolSelector`, `scope?: ScopeSelector`, `includeContext?: boolean = false`, `callersLimit?: int = 100`, `callSitesPerCallerLimit?: int = 100`. | `CallerSearchData { symbol: SymbolReference, callers: BoundedCollection<CallerInfo { caller: SymbolReference, callSites: BoundedCollection<CallerSiteInfo { location: ResolvedLocation, context?: string }> }> }` |
...
| `analyze-control-flow` | Existing | Q | **Analyze Control Flow**. Returns reachability, exit paths and return behaviour for an exact complete statement or contiguous statement range. Partial statements and cross-body ranges are rejected. | `location: LocationSelector`, `exitsLimit?: int = 100`, `returnsLimit?: int = 100`. | `ControlFlowAnalysisData { region: ResolvedLocation, entryReachable: boolean, exitReachable: boolean, exits: BoundedCollection<ControlFlowExit>, returns: BoundedCollection<ResolvedLocation> }` |
| `analyze-data-flow` | Existing | Q | **Analyze Data Flow**. Returns reads, writes, data in/out and captured variables for an exact expression, complete statement or contiguous statement range. Partial statements and cross-body ranges are rejected. | `location: LocationSelector`, `symbolsPerCategoryLimit?: int = 50`. The limit applies independently to each returned symbol category. | `DataFlowAnalysisData { region: ResolvedLocation, variablesDeclared: BoundedCollection<SymbolReference>, readInside: BoundedCollection<SymbolReference>, writtenInside: BoundedCollection<SymbolReference>, dataFlowsIn: BoundedCollection<SymbolReference>, dataFlowsOut: BoundedCollection<SymbolReference>, captured: BoundedCollection<SymbolReference> }` |
| `get-operation-tree` | New | Q | **Get Operation Tree**. Returns a bounded typed `IOperation` tree containing operation metadata and exact source pointers rather than embedded syntax. | `location: LocationSelector`, `maxDepth?: int = 8 (maximum 24)`, `nodesLimit?: int = 200 (maximum 2000)`. | `OperationTreeData { root?: OperationNode { kind, type?, hasConstantValue, location: ResolvedLocation, truncated, children: OperationNode[] }, truncated: boolean }` |
| `get-control-flow-graph` | New | Q | **Get Control Flow Graph**. Returns basic blocks, branches and regions for an executable symbol or body. Block operations contain metadata and exact source pointers rather than embedded syntax. | `symbol?: SymbolSelector`, `location?: LocationSelector`, `maxBlocks?: int = 64`, `maxRegions?: int = 32`, `maxOperationsPerBlock?: int = 32`. Exactly one of `symbol` or `location` is required. | `ControlFlowGraphData { owner: SymbolReference, blocks: BoundedCollection<BasicBlockInfo { operations: BoundedCollection<BasicBlockOperationInfo { kind, type?, location: ResolvedLocation }> }>, regions: BoundedCollection<FlowRegionInfo> }` |
...
| `find-duplicate-code` | New | Q | **Find Duplicate Code**. Groups identical executable blocks by their normalized statement sequence for review. It returns exact symbol and location pointers rather than embedding the duplicated source and is advisory rather than an automatic refactoring. | `scope?: ScopeSelector = Solution`, `minimumStatements?: int = 3`, `groupsLimit?: int = 25`, `occurrencesPerGroupLimit?: int = 100`. | `DuplicateCodeData { groups: BoundedCollection<DuplicateCodeGroup { statementCount: int, occurrences: BoundedCollection<DuplicateCodeOccurrence { symbol: SymbolReference, location: ResolvedLocation }> }> }` |
...
| `find-dependency-cycles` | New | Q | **Find Dependency Cycles**. Detects dependency cycles at project, namespace or type granularity. | `scope?: ScopeSelector = Solution`, `granularity: Project \| Namespace \| Type`, `cyclesLimit?: int`, `nodesLimit?: int = 25000 (maximum 100000)`, `edgesLimit?: int = 100000 (maximum 500000)`. | `DependencyCyclesData { cycles: BoundedCollection<DependencyCycle { nodes: GraphNode[] }> }` |
...
| `analyze-async` | New | Q | **Analyze Async**. Returns the bundled AsyncFixer01–AsyncFixer06 diagnostics and compiler diagnostic CS4014 for a selected scope. The bundled analyzers run without changing the target project's package references and respect its analyzer configuration. | `scope?: ScopeSelector = Solution`, `findingsLimit?: int`. | `AsyncAnalysisData { findings: BoundedCollection<AsyncFinding { diagnostic: DiagnosticInfo }> }` |
...
Structural tools explicitly state that the target SDK-style project must include the resulting source file through its own conventions. The server never edits a project file. Every mutation input below additionally requires `expectedSnapshot: SnapshotPrecondition`; action-staging inputs supply it both explicitly and through their snapshot-bound action reference.
...
| `add-obsolete-attribute` | New | M | **Add Obsolete Attribute**. Adds an `Obsolete` attribute to a declaration affected by use or override of an obsolete API through the validated Roslyn code-fix provider. | `location: LocationSelector`, `expectedSnapshot: SnapshotPrecondition`. |
...
| `assign-out-parameters` | New | M | **Assign Out Parameters**. Assigns unassigned `out` parameters through the validated Roslyn providers, preferring assignment at method start and falling back to assignment above the return where required. | `location: LocationSelector`, `expectedSnapshot: SnapshotPrecondition`. |
| `declare-as-nullable` | New | M | **Declare As Nullable**. Makes the declaration associated with a supported nullability compiler warning nullable through the validated Roslyn code-fix provider. | `location: LocationSelector`, `expectedSnapshot: SnapshotPrecondition`. |
| `fix-incorrect-constraint` | New | M | **Fix Incorrect Constraint**. Replaces an invalid `enum` or `delegate` generic constraint with its supported form through the validated Roslyn code-fix provider. | `location: LocationSelector`, `expectedSnapshot: SnapshotPrecondition`. |
| `fix-return-type` | New | M | **Fix Return Type**. Changes a void or task-like return type to match the returned expression through the validated Roslyn code-fix provider. | `location: LocationSelector`, `expectedSnapshot: SnapshotPrecondition`. |
...
| `format-document` | Existing | M | **Format Document**. Formats one source document using loaded workspace options. | `document: DocumentSelector`, `range?: TextSpanRange`. |
...
| --- | --- | --- | --- | --- | --- |
| `list-code-actions` | Host-owned Code Action | QR | **List Code Actions**. Lists bounded Roslyn Code Fixes and refactorings for a snapshot-bound document, selection or caret after exception-policy filtering. | `document: DocumentSelector`, `range?: TextSpanRange`, `expectedSnapshot: SnapshotPrecondition`, `kinds: CodeFixes \| Refactorings \| All`, `diagnosticIds?: string[]`, `limit?: int = 50`, `workspace?: WorkspaceSelector`. | `CodeActionListData { actions: BoundedCollection<CodeActionListItem> }` |
...
The JSON Schema records the shape of tool arguments. The tool executor still performs the following runtime checks:
...
- A mutation has an active, non-conflicted transaction with remaining revision capacity.
- Query tools resolve against the staged solution while a transaction is active. In `TransactionConflicted`, normal query and mutation work is rejected; only `transaction-preview` and `transaction-rollback` remain available.
- A required snapshot precondition matches the effective solution before a prior location, symbol or action reference is used.
- Selectors resolve uniquely against the current effective solution.
- The relevant workspace-input manifest is fresh before semantic work. A monitored manifest-input change or fallback metadata verification can invalidate the loaded snapshot.
- A candidate solution contains only allow-listed regular source-document text changes and validated regular source-document create/delete operations. All project, reference, option, analyzer, additional-document, and document metadata changes are rejected.
- Every created, deleted or relocated document has a canonical owned path.
- An action reference is rejected with `ActionExpired` when it is unknown, expired, evicted or snapshot-invalid, or with `ActionAmbiguous` when re-running its provider does not yield exactly one matching action. The server retains replay recipes but no provider-created `CodeAction` instances between requests.
- Tool-specific Roslyn semantic preconditions are satisfied.

## Cancellation and Commit Recovery

Queries and candidate-solution construction honour cancellation before they alter server state. Revision staging, rollback, and transaction-history have a short state-transition boundary: a cancellation observed before that boundary leaves state unchanged; one observed after it does not undo the completed transition. The client can confirm the resulting revision through `workspace-status`.

Commit derives and validates the complete source-file plan before touching a target. It stores a versioned manifest, exact binary backups and exact staged contents under `StateDirectory/recovery/<commit-id>`, then durably transitions through `Prepared`, `Applying` and `Committed`. Cancellation is honoured before `Applying`; application and restoration are non-cancellable after that point.

An authoritative per-workspace-root inter-process lock serialises commits across local server instances. Each target is hash-checked immediately before it is changed. A failed or interrupted commit restores every non-divergent target to its original bytes and existence state, removes transaction-created files and empty directories, and records `RecoveryConflict` rather than overwriting a file changed by another local tool. Restoration I/O failures remain as `RecoveryIncomplete` and are retried during startup before MCP transport is started. `server-status`, `RecoveryStatus`, and their JSON remain unchanged.

`WorkspaceIdentity` reports both `loadedPath`, the `.sln`, `.slnx` or `.csproj` loaded by Roslyn, and `workspaceRoot`, the repository-level transaction and coordination boundary. An explicit `workspaceRoot` is validated as an existing ancestor of every loaded project and document. When omitted, the server uses the nearest `.git` directory or worktree file, falling back to the loaded file's directory.

Per-instance files under `<workspace-root>/.vs/roslyn-workbench-mcp/instances` are advisory only. They publish workspace/transaction/commit state and produce a `WorkspaceInUse` warning for another live local instance; they are never consulted as commit or recovery evidence. The files use a versioned JSON shape containing `instanceId`, `loadedPath`, `workspaceRoot`, `workspaceState`, `transactionRevision`, `commitId`, and `commitPhase`. `workspace-status` enumerates live files for the selected workspace and returns other instances through its `instances` collection. Stale files are removed when their ownership handle can be acquired; malformed live files still cause the open warning but are omitted from the structured query result.

`WorkspaceInUse`, unavailable instance status and unreadable live-instance data all require the same conservative agent response: treat the workspace as query-only, use it only when necessary, expect results to become stale, and coordinate mutation ownership before starting a transaction. This remains advisory; the Host does not treat instance files as an authoritative mutation lock or reject `transaction-start` solely because of their state.

Atomic replacement flushes staged file contents before publication. Windows uses `MoveFileExW` with replace and write-through flags; Unix uses a same-volume rename followed by `fsync` of the parent directory. Delete application uses the same durable move primitive to create a transaction-owned tombstone. The authoritative workspace-root lock uses the CLR `FileStream.Lock` byte-range primitive and is released by the operating system when an owning process terminates. Lock-file open failures remain distinct from lock contention. These guarantees assume a local filesystem that honours the corresponding operating-system durability primitives; faulty storage hardware and filesystems that falsely acknowledge flushes remain outside the contract.

## Original Server Mapping

The `Existing` tools retain the conceptual capability from `JoshuaRamirez/RoslynMcpServer`. Its request DTOs used absolute solution and source-file paths plus optional line/column and `preview` arguments. This server replaces those with one loaded workspace, shared selectors, staged mutations and common structured results. Its retained `maxResults` behaviour informs the optional per-collection integer limits, but the unbounded defaults are not retained.
