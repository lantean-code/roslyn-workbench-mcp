...
public sealed class IntroduceUsingStatementToolIntegrationTests
{
    [Fact]
    public async Task GIVEN_ActiveTransaction_WHEN_ExecutingTool_THEN_ShouldStageMutation()
    {
        ...
        var result = await BundledCoreToolTestHarness.ExecuteMutationAsync(coordinator, "introduce-using-statement", target, RefactoringRequestFactory.CreateLocationRequest(fixture.GetSelection("var stream = new MemoryStream();"), openResult));
        ...
        result.StagedResult.Data!.Operation.Should().Be("introduce-using-statement");
