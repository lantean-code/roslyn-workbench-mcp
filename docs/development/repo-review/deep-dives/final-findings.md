...
| 13 | In progress | Scenario-runner evidence integrity | RWMCP-038 → RWMCP-034 → RWMCP-036 → RWMCP-035 → RWMCP-037 | RWMCP-038, RWMCP-034 and RWMCP-035 are completed and revalidated: one runner owns an entire repository cache, preparation restores only EF Core's three declared sentinel-file effects, both pre/post certification reject any remaining tracked drift with exact paths, and durable commits use direct commit-scoped EventPipe collection with phase and atomic-replacement retry evidence. RWMCP-036 is rejected because incomplete development-run evidence should be discarded and rerun after correction. Next enforce cancellation outcomes. |
| 14 | Not started | Protocol continuation contract | RWMCP-040 | Replace the leaked `RequiredAction` enum token with an unambiguous public continuation contract that distinguishes an exact tool invocation from general recovery guidance. Omit absent correlation identifiers and retain non-null identifiers for unexpected correlated failures. Revalidate every server-owned, Code Action and plugin failure serializer and representative published responses. |
...
- Severity: P2
- Confidence: High
- Exact file and line range: `src/Roslyn.Workbench.Mcp.Abstractions/Workspace/Results/RequiredAction.cs:9-61`; `src/Roslyn.Workbench.Mcp/Contracts/Results/ToolResultEnvelopeSerializer.cs:65-102`; `src/Roslyn.Workbench.Mcp/Contracts/Results/ToolError.cs:5-22`
