...
public sealed class UseExplicitTypeToolIntegrationTests
{
    [Fact]
    public async Task GIVEN_ActiveTransaction_WHEN_ExecutingTool_THEN_ShouldStageMutation()
    {
        ...
        var target = new UseExplicitTypeTool();

        openResult.Outcome.Should().Be(ToolOutcome.Succeeded);

        var result = await BundledCoreToolTestHarness.ExecuteMutationAsync(coordinator, "use-explicit-type", target, RefactoringRequestFactory.CreateLocationRequest(fixture.GetCursor("var explicitBuilder", 0, "var".Length), openResult));
        ...
        result.StagedResult.Data!.Operation.Should().Be("use-explicit-type");
