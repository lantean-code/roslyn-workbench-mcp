...
**Remediation outcome:** Deliberate fallback routing failures now use an internal `RoslynWorkbenchMcpProtocolException` marker carrying the intended MCP error code, and `UnhandledToolExceptionFilter` propagates only that Host-owned marker to the SDK dispatcher. Unknown tools and unsupported task-augmented plugin calls therefore reach clients as `InvalidParams` without logging, captured-error retention or adapter invocation, while ordinary `McpProtocolException`, base `McpException` and other unexpected exceptions from direct tools, Code Actions or plugins remain inside the correlated, redacted `UnhandledException` path. The filter's elicitation capability projection retains its three-state distinction between unavailable capabilities, known unsupported elicitation and supported elicitation. New `RWMCP023` authoring diagnostics reject direct, variable, derived and typed-rethrow protocol exceptions in declared plugin assemblies that reference the MCP SDK; the packaged external-consumer integration proves that the shipped analyser activates against the real pinned SDK. Unit tests passed 505/505, Host integration tests passed 77/77, plugin-analyser tests passed 70/70, the solution build completed without warnings or errors, and changed-file `latest-all` analysis was clean apart from existing CA1812 warnings in unchanged dynamically instantiated plugin fixtures. The first fresh context-free Review Agent identified that the initial blanket `McpProtocolException` passthrough weakened the runtime trust boundary; the Host-owned marker, capture regression and packaged-consumer coverage corrected that defect. A second fresh context-free Review Agent pass returned no findings. Its residual risks are that static analysis cannot detect protocol exceptions hidden behind broader static types or thrown by dependencies and that the runtime probe is in-process rather than a published-Host external-plugin scenario; runtime capture remains authoritative for those cases. No acceptance artefact changed, so the acceptance wrapper was not required. The user gave final confirmation on 2026-08-14.
