...
public sealed class InvertIfToolIntegrationTests
{
    [Fact]
    public async Task GIVEN_ActiveTransaction_WHEN_ExecutingTool_THEN_ShouldStageMutation()
    {
        ...
        var result = await BundledCoreToolTestHarness.ExecuteMutationAsync(coordinator, "invert-if", target, RefactoringRequestFactory.CreateLocationRequest(fixture.GetLocation("if (left > 0)"), openResult));
        ...
        result.StagedResult.Data!.Operation.Should().Be("invert-if");
