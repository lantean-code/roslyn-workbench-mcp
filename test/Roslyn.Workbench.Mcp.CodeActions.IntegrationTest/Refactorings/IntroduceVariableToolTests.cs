...
public sealed class IntroduceVariableToolIntegrationTests
{
    [Fact]
    public async Task GIVEN_ActiveTransaction_WHEN_ExecutingTool_THEN_ShouldStageMutation()
    {
        ...
        var result = await BundledCoreToolTestHarness.ExecuteMutationAsync(coordinator, "introduce-variable", target, new IntroduceVariableRequest
        {
            Selection = fixture.GetLocation("1 + 1"),
            Kind = IntroduceVariableKind.LocalConstant,
        ...
        result.StagedResult.Data!.Operation.Should().Be("introduce-variable");
