...
The later live-build sequence keeps a Workspace open while the target repository builds, then repeats a warmed semantic query and inspects `workspace-status`. Its first diagnostic run exposed generated AssemblyInfo documents under `obj` as metadata-polling false positives in GuardClauses and EF Core. Generated output is now excluded consistently from watcher and Roslyn-document polling. Confirming native-WSL-filesystem runs remained `Ready`, repeated the query successfully, restored each checkout and left no recovery or coordination state. Build durations were 1,280.74 ms for GuardClauses, 1,705.81 ms for Serilog and 18,890.66 ms for EF Core. Evidence: `artifacts/performance/results/20260724-174543-guardclauses-a1b7347c185a4025a148f23b97355e22`, `artifacts/performance/results/20260724-174604-serilog-a5c3f36f749e4bfba49478721d873e57` and `artifacts/performance/results/20260724-174623-efcore-0b46fcb87b58455aa582f4bf36345897`.

Native Windows confirmed the same boundary after directory-level watcher noise was removed. GuardClauses, Serilog and EF Core remained `Ready` without an external-change diagnostic; every post-build query matched its baseline response. Their builds completed in 1,247.36 ms, 1,760.84 ms and 16,791.54 ms respectively. Each repository returned to its pinned commit, every Host exited normally without forced termination, and validation found no recovery files, new Workspace state files or other issues. Evidence: `artifacts/performance/results/20260724-184730-guardclauses-417f41d7d7fe45519e952b57cafb53b4`, `artifacts/performance/results/20260724-184739-serilog-40a3d782f6c84b7a943fae375e2b5241` and `artifacts/performance/results/20260724-184752-efcore-b78371f142a04e0ead113ad82a10fdbf`.

The generated-tree policy was then tightened from global directory-name exclusions to project-evaluated roots. It now evaluates MSBuild output, intermediate, publish, package and central-artifacts properties once per physical project; supplements them with Roslyn's resolved output paths; retains exact generated build inputs; and protects loaded solution/project paths from an unsafe broad exclusion. Focused integration evidence covers custom intermediate and central artifacts roots while proving an ordinary source tree named `obj` remains tracked. The published WSL acceptance suite passed 41 of 41 tests. Fresh native-WSL-filesystem live-build runs kept all three Workspaces `Ready`, returned response hashes identical to their warmed baselines and left repository, Host and state validation clean. Build durations were 2,322.37 ms for GuardClauses, 1,610.06 ms for Serilog and 17,573.94 ms for EF Core. Evidence: `artifacts/performance/results/20260724-192645-guardclauses-18aa94e2053e48a1befba7441abfd983`, `artifacts/performance/results/20260724-192656-serilog-de9454b0860c4d6aaeffa0a36e503dcd` and `artifacts/performance/results/20260724-192708-efcore-0aa78be3709e481b94c647836c7cd90b`.

Native Windows then exposed an intermittent `FileSystemWatcher` buffer overflow during the EF Core live build. The watcher had been classifying and publishing every notification directly on the native callback path, reducing the rate at which .NET could drain the operating-system buffer. The monitor now uses the maximum 64 KiB native buffer and hands lightweight notification values immediately to an unbounded channel. One ordered consumer applies manifest and generated-tree policy, publishes the first relevant change and stops monitoring; request-time validation waits for notifications already accepted by managed code so the asynchronous hand-off cannot transiently expose a stale Workspace as `Ready`. Existing metadata polling and classified watcher-error invalidation remain as fallbacks.

The bounded watcher-stress sequence creates, rewrites twice and deletes 20,000 generated files, producing 80,000 filesystem operations beneath an evaluated artifact root. Native WSL completed the burst in 1,565.50 ms, retained `Ready`, completed the post-stress query and increased final Host working set by approximately 5.9 MiB. Native Windows completed it in 16,175.83 ms, retained `Ready`, completed the query, finished approximately 7.6 MiB below its starting working set and peaked approximately 26.6 MiB above it. Both runs shut down normally and left the pinned checkout, recovery state and Workspace state clean. Evidence: `artifacts/performance/results/20260724-202342-efcore-e2c74217aafc4bfd899fd56067c5fb16` and `artifacts/performance/results/20260724-202550-efcore-16c4ece1fc5247f495bec67142d26da1`.

Three consecutive native Windows EF Core live builds then completed in 15,459.16 ms, 11,501.27 ms and 10,517.53 ms without a watcher error or false stale transition. Every Workspace remained `Ready`, every post-build query exactly matched its warmed response, final Host working set was lower than its pre-build value and validation found no checkout, recovery, state-file or shutdown issue. This repeated real-build evidence confirms that the channel hand-off resolves the observed practical overflow without requiring a Git-index-style change tracker or per-query filesystem scan. Evidence: `artifacts/performance/results/20260724-202708-efcore-3a0e308de5564f94a3bbc193b24b353a`, `artifacts/performance/results/20260724-202753-efcore-89f0b5f3bd7e4097a224636421fbfe5b` and `artifacts/performance/results/20260724-202838-efcore-8fc5a84b6a824e768297c04149ce2ac3`.
