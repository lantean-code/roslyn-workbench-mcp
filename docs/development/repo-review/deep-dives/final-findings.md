...
The repository should not proceed through the final v1 release gate until the two P1 findings have been remediated and revalidated. RWMCP-014 can physically delete a linked source file that the promoted Solution still references. RWMCP-031 can send SDK-enriched stack, module and runtime data that was absent from the user-reviewed Sentry payload. The P2/P3 set contains important correctness, compatibility, cancellation, resource and operational issues but does not invalidate the otherwise coherent architecture.

## Suggested implementation order

Treat each row as an individual implementation work step. Complete and revalidate the findings in the listed internal order before starting a dependent step. Steps 1A and 1B are independent P1 tracks and may proceed in parallel; both block the final v1 release gate. Other steps may be parallelised only when they do not touch the same contracts or tests, but the sequence below minimises rework from foundational contract and persistence changes.

| Step | Status | Group | Findings in implementation order | Dependency rationale and completion gate |
| --- | --- | --- | --- | --- |
| 1A | Not started | Linked-source transaction safety | RWMCP-014 → RWMCP-019 | Fix asymmetric linked deletion first because it is the data-loss/consistency blocker and establishes the canonical physical-path grouping used by linked reconciliation. Then correct adjacent text-change merging against the same linked-document fixtures. Gate: candidate processing and commit planning must reject or normalise asymmetric deletion, preserve surviving linked files and accept safe adjacent changes. |
| 1B | Not started | Sentry outbound allow-list | RWMCP-031 | Independent P1 trust-boundary fix. Enforce the reviewed data shape after SDK processing and prove the final serialised envelope, not merely the pre-SDK event. Gate: the approved preview and outbound diagnostic content must have the same allow-listed categories. |
| 2 | Not started | Snapshot and selector contract foundations | RWMCP-009 → RWMCP-027 → RWMCP-012 → RWMCP-010 | Change public snapshot identity before extending validation and consumers so all downstream fixtures use the final identity shape. Connect nested selector validation at the Host boundary, then centralise non-throwing path normalisation through that validated path, and finally align solution-hierarchy path identity with the resulting canonical comparer/root rules. Gate: restart replay, contradictory selectors, malformed paths, broader workspace roots and filesystem casing are covered across real adapters. |
| 3 | Not started | Workspace certification and lifecycle | RWMCP-008 → RWMCP-013 → RWMCP-011 | Establish a certified Solution/manifest publication protocol across open, reload and commit promotion first. Once reload publication is structurally final, republish its `Ready` advisory state. Add cancellation checks to batch target-framework evaluation without weakening the certified Workspace lease boundary. Gate: no inconsistent Ready snapshot is publishable, reload clears external advisory state and cancellation releases large-solution query leases at project boundaries. |
| 4 | Not started | Recovery payload and persistence limits | RWMCP-016 → RWMCP-018 → RWMCP-017 | Persist and restore source metadata before finalising recovery budgets because metadata changes the manifest/artifact representation. Add aggregate planning limits next, then translate all final per-item and aggregate limit failures at the commit boundary. Gate: replacement/recovery preserves supported metadata, planning is cumulatively bounded and every supported-limit rejection is structured while the transaction remains recoverable. |
| 5 | Not started | Protocol schema pipeline | RWMCP-028 → RWMCP-021 | Correct the authoritative Full output schema before using that same schema generation as plugin admission preflight. Gate: representative success/no-change/failure payloads validate against published schemas, and one unsupported plugin contract disables only that plugin before DI publication. |
| 6 | Not started | Plugin catalogue admission | RWMCP-020 → RWMCP-022 | Apply final tool-name validation before composition/collision admission, then reject asynchronous configuration implementations before invocation and freeze. Both changes belong to the per-plugin enable/disable boundary. Gate: invalid names and suppressed-analyser `async void Configure` binaries produce deterministic disabled status without affecting the remaining catalogue. |
| 7 | Not started | Plugin stdio containment | RWMCP-030 | Reserve the MCP transport stream before any extension construction or invocation and provide a safe diagnostic path. Gate: a packaged plugin may write during construction, configuration and invocation without corrupting initialisation or a subsequent MCP call. |
...
- Commit tests do not cover per-artifact/manifest limit translation or aggregate plan budgets.
...
- Runtime composition tests do not suppress RWMCP003 and verify rejection of an `async void Configure` binary.
...
- Published and integration plugin fixtures do not write to `Console.Out` during construction, configuration, conversion or invocation.
