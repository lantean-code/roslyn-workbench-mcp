...
public sealed class UseNamedArgumentsToolIntegrationTests
{
    [Fact]
    public async Task GIVEN_ActiveTransaction_WHEN_ExecutingTool_THEN_ShouldStageMutation()
    {
        ...
        var result = await BundledCoreToolTestHarness.ExecuteMutationAsync(coordinator, "use-named-arguments", target, RefactoringRequestFactory.CreateUseNamedArgumentsRequest(fixture.GetCursor("Sum(1, 2)", 0, 4), openResult, includeTrailingArguments: false));
        ...
        result.StagedResult.Data!.Operation.Should().Be("use-named-arguments");
