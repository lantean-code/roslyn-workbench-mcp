...
| `CollectionLimit` | `maxResults?: int`. Omitted means the server startup `DefaultMaxResults`, initially `100`. Must be zero or greater. `0` means “return none from this collection”. There is no cursor in v1; a larger request recomputes the ordered result set. |
...
| `get-solution-structure` | New plugin | Q | **Get Solution Structure**. Returns solution folders, projects, target frameworks and direct project relationships. | `includeDocuments?: boolean = false`, `foldersLimit?: CollectionLimit`, `projectsLimit?: CollectionLimit`. | `SolutionStructureData { solutionPath, folders: BoundedCollection<SolutionFolderInfo>, projects: BoundedCollection<ProjectStructureInfo> }` |
| `get-project-details` | New plugin | Q | **Get Project Details**. Returns project properties, optional document inventory, direct references, analyzers and compilation options. | `project: ProjectSelector`, `includeDocuments?: boolean = false`, `documentsLimit?: CollectionLimit`, `projectReferencesLimit?: CollectionLimit`, `metadataReferencesLimit?: CollectionLimit`, `analyzersLimit?: CollectionLimit`. | `ProjectDetailsData { project: ProjectInfo, documents?: BoundedCollection<DocumentReference>, projectReferences: BoundedCollection<ProjectReferenceInfo>, metadataReferences: BoundedCollection<MetadataReferenceInfo>, analyzers: BoundedCollection<AnalyzerInfo>, compilationOptions: CompilationOptionsInfo }` |
...
| `get-symbol-members` | New | Q | **Get Symbol Members**. Lists declared members and optionally inherited or explicit-interface members. | `symbol: SymbolSelector`, `includeInherited?: boolean = false`, `includeExplicitInterface?: boolean = false`, `membersLimit?: CollectionLimit`. | `SymbolMembersData { symbol: SymbolReference, members: BoundedCollection<SymbolReference> }` |
| `get-symbol-attributes` | New | Q | **Get Symbol Attributes**. Returns declared and inherited attributes with constructor and named arguments. | `symbol: SymbolSelector`, `includeInherited?: boolean = false`, `attributesLimit?: CollectionLimit`. | `SymbolAttributesData { symbol: SymbolReference, attributes: BoundedCollection<AttributeInfo> }` |
...
| `find-references` | Existing | Q | **Find References**. Finds source references, optionally including declarations and access classification. | `symbol: SymbolSelector`, `scope?: ScopeSelector`, `includeDefinitions?: boolean = true`, `includeContext?: boolean = false`, `referencesLimit?: CollectionLimit`. | `ReferenceSearchData { symbol: SymbolReference, references: BoundedCollection<ReferenceLocation> }` |
| `find-callers` | Existing | Q | **Find Callers**. Returns direct source call sites and containing symbols. | `symbol: SymbolSelector`, `scope?: ScopeSelector`, `includeContext?: boolean = false`, `callersLimit?: CollectionLimit`. | `CallerSearchData { symbol: SymbolReference, callers: BoundedCollection<CallerInfo> }` |
| `find-callees` | New | Q | **Find Callees**. Returns symbols directly invoked by a method or selected executable body. | `symbol?: SymbolSelector`, `location?: LocationSelector`, `includeIndirect?: boolean = false`, `calleesLimit?: CollectionLimit`. Exactly one of `symbol` or `location`. | `CalleeSearchData { source: SymbolReference, callees: BoundedCollection<SymbolReference> }` |
| `find-implementations` | Existing | Q | **Find Implementations**. Finds implementations of an interface or abstract member. | `symbol: SymbolSelector`, `scope?: ScopeSelector`, `implementationsLimit?: CollectionLimit`. | `ImplementationSearchData { symbol: SymbolReference, implementations: BoundedCollection<SymbolReference> }` |
| `find-overrides` | New | Q | **Find Overrides**. Finds overrides of a virtual or abstract member. | `symbol: SymbolSelector`, `scope?: ScopeSelector`, `overridesLimit?: CollectionLimit`. | `OverrideSearchData { symbol: SymbolReference, overrides: BoundedCollection<SymbolReference> }` |
| `find-derived-types` | New | Q | **Find Derived Types**. Finds derived types with optional depth and project filters. | `symbol: SymbolSelector`, `scope?: ScopeSelector`, `maxDepth?: int`, `derivedTypesLimit?: CollectionLimit`. | `DerivedTypesData { baseType: SymbolReference, derivedTypes: BoundedCollection<TypeHierarchyNode> }` |
| `get-type-hierarchy` | Existing | Q | **Get Type Hierarchy**. Returns base types, implemented interfaces and optional derived types. | `symbol: SymbolSelector`, `includeDerived?: boolean = true`, `maxDepth?: int`, `baseTypesLimit?: CollectionLimit`, `interfacesLimit?: CollectionLimit`, `derivedTypesLimit?: CollectionLimit`. | `TypeHierarchyData { type: SymbolReference, baseTypes: BoundedCollection<SymbolReference>, interfaces: BoundedCollection<SymbolReference>, derivedTypes?: BoundedCollection<TypeHierarchyNode> }` |
| `find-overloads` | New | Q | **Find Overloads**. Returns overloads and parameter signatures for a method or constructor. | `symbol: SymbolSelector`, `overloadsLimit?: CollectionLimit`. | `OverloadSearchData { symbol: SymbolReference, overloads: BoundedCollection<CallableSignature> }` |
| `get-partial-declarations` | New | Q | **Get Partial Declarations**. Returns every declaration of a partial type or method. | `symbol: SymbolSelector`, `declarationsLimit?: CollectionLimit`. | `PartialDeclarationsData { symbol: SymbolReference, declarations: BoundedCollection<ResolvedLocation> }` |
| `get-symbol-dependencies` | New | Q | **Get Symbol Dependencies**. Returns symbols, types and assemblies directly used by a symbol. | `symbol: SymbolSelector`, `includeAssemblies?: boolean = true`, `dependenciesLimit?: CollectionLimit`. | `SymbolDependenciesData { symbol: SymbolReference, dependencies: BoundedCollection<DependencyInfo> }` |
| `get-symbol-dependents` | New | Q | **Get Symbol Dependents**. Returns symbols that directly depend on a symbol. | `symbol: SymbolSelector`, `scope?: ScopeSelector`, `dependentsLimit?: CollectionLimit`. | `SymbolDependentsData { symbol: SymbolReference, dependents: BoundedCollection<SymbolReference> }` |

## Analysis and Architecture (16)

| Tool | Source | Behaviour | Title and description | Input parameters | Success payload shape |
|---|---|---|---|---|---|
| `get-diagnostics` | Existing | Q | **Get Diagnostics**. Returns compiler and configured analyzer diagnostics for a scope. | `scope?: ScopeSelector`, `severities?: string[]`, `ids?: string[]`, `diagnosticsLimit?: CollectionLimit`. | `DiagnosticsData { diagnostics: BoundedCollection<DiagnosticInfo> }` |
| `get-code-metrics` | Existing | Q | **Get Code Metrics**. Returns projected logical lines, cyclomatic complexity, nesting depth, type coupling and a derived maintainability score for a symbol or scope. | `scope?: ScopeSelector = Solution`, `symbol?: SymbolSelector`, `includeChildren?: boolean = false`, `metricsLimit?: CollectionLimit`, `expectedSnapshot?: SnapshotPrecondition` required for location-based symbol selectors. | `CodeMetricsData { metrics: BoundedCollection<MetricInfo { symbol?: SymbolReference, location?: ResolvedLocation, logicalLines: int, cyclomaticComplexity: int, maxNestingDepth: int, coupling: int, maintainabilityIndex: int }> }` |
...
| `find-unused-symbols` | New | Q | **Find Unused Symbols**. Identifies candidate unused locals and members from compiler-unused diagnostics, with confidence reasons. | `scope?: ScopeSelector = Solution`, `includeInternal?: boolean = false`, `excludeGenerated?: boolean = true`, `candidatesLimit?: CollectionLimit`. | `UnusedSymbolsData { candidates: BoundedCollection<UnusedSymbolCandidate { symbol: SymbolReference, location?: ResolvedLocation, confidence: string, reasons: string[] }> }` |
| `find-duplicate-code` | New | Q | **Find Duplicate Code**. Groups identical executable blocks by their normalized statement sequence for review. It is advisory and does not refactor code. | `scope?: ScopeSelector = Solution`, `minimumStatements?: int = 3`, `groupsLimit?: CollectionLimit`. | `DuplicateCodeData { groups: BoundedCollection<DuplicateCodeGroup { statementCount: int, occurrences: DuplicateCodeOccurrence[] }> }` |
| `get-dependency-graph` | New | Q | **Get Dependency Graph**. Builds a bounded dependency graph at a selected granularity. | `scope?: ScopeSelector = Solution`, `granularity: Project|Namespace|Type|Symbol`, `maxDepth?: int = 3`, `nodesLimit?: CollectionLimit`, `edgesLimit?: CollectionLimit`. | `DependencyGraphData { nodes: BoundedCollection<GraphNode>, edges: BoundedCollection<GraphEdge> }` |
| `find-dependency-cycles` | New | Q | **Find Dependency Cycles**. Detects dependency cycles at project, namespace or type granularity. | `scope?: ScopeSelector = Solution`, `granularity: Project|Namespace|Type`, `cyclesLimit?: CollectionLimit`. | `DependencyCyclesData { cycles: BoundedCollection<DependencyCycle { nodes: GraphNode[] }> }` |
| `get-change-impact` | New | Q | **Get Change Impact**. Estimates blast radius using references, callers, overrides, implementations and public surface. | `symbol: SymbolSelector`, `scope?: ScopeSelector`, `locationsLimit?: CollectionLimit`. | `ChangeImpactData { symbol: SymbolReference, impact: ImpactSummary, locations: BoundedCollection<ReferenceLocation> }` |
| `get-api-surface` | New | Q | **Get API Surface**. Describes exported API symbols for a solution, project, namespace or type. | `scope: ScopeSelector`, `minimumAccessibility?: Public|Protected|Internal = Public`, `includeObsolete?: boolean = true`, `symbolsLimit?: CollectionLimit`. | `ApiSurfaceData { symbols: BoundedCollection<ApiSymbolInfo> }` |
| `get-test-impact` | New | Q | **Get Test Impact**. Identifies likely impacted tests using built-in test-like type and method naming conventions. | `symbol: SymbolSelector`, `testScope?: ScopeSelector = Solution`, `includeReasons?: boolean = false`, `testsLimit?: CollectionLimit`. | `TestImpactData { symbol: SymbolReference, tests: BoundedCollection<TestImpactInfo { test: SymbolReference, location?: ResolvedLocation, reasons?: string[] }> }` |
| `analyze-nullability` | New | Q | **Analyze Nullability**. Returns nullable-flow issues and unsafe dereferences from compiler nullability diagnostics. | `scope?: ScopeSelector = Solution`, `location?: LocationSelector`, `findingsLimit?: CollectionLimit`, `expectedSnapshot?: SnapshotPrecondition` required for location-based selectors. | `NullabilityAnalysisData { findings: BoundedCollection<NullabilityFinding { diagnostic: DiagnosticInfo }> }` |
| `analyze-async` | New | Q | **Analyze Async**. Identifies supported async antipatterns using syntax and operation analysis. | `scope?: ScopeSelector = Solution`, `findingsLimit?: CollectionLimit`. | `AsyncAnalysisData { findings: BoundedCollection<AsyncFinding { kind: string, symbol?: SymbolReference, location?: ResolvedLocation, message: string }> }` |
| `analyze-disposables` | New | Q | **Analyze Disposables**. Identifies candidate undisposed local `IDisposable` or `IAsyncDisposable` values. Findings are advisory. | `scope?: ScopeSelector = Solution`, `findingsLimit?: CollectionLimit`. | `DisposableAnalysisData { findings: BoundedCollection<DisposableFinding { kind: string, symbol?: SymbolReference, type?: TypeInfo, location?: ResolvedLocation, message: string }> }` |
...
optional `CollectionLimit` design, but the unbounded defaults are not retained.
