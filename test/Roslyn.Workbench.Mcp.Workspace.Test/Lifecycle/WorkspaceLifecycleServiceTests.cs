...
public sealed class WorkspaceLifecycleServiceTests : IDisposable
{
    ...
    private readonly Mock<IWorkspaceReadOnlyDocumentValidator> _readOnlyDocumentValidator;
    private readonly Mock<IWorkspaceInputCertification> _inputCertification;
    ...
    public WorkspaceLifecycleServiceTests()
    {
        ...
        _msBuildPropertiesResolver = new Mock<IWorkspaceMsBuildPropertiesResolver>();
        _msBuildPropertiesResolver
            .Setup(item => item.Resolve(It.IsAny<WorkspaceMsBuildProperties?>()))
        ...
        _workspacePathNormalizer = new Mock<IWorkspacePathNormalizer>();
        ...
        _readOnlyDocumentValidator = new Mock<IWorkspaceReadOnlyDocumentValidator>();
        _readOnlyDocumentValidator
            .Setup(item => item.ValidateAsync(
                It.IsAny<Solution>(),
                It.IsAny<string>(),
                It.IsAny<CancellationToken>()))
            .ReturnsAsync(WorkspaceReadOnlyDocumentValidationStatus.Valid);
        _inputCertification = new Mock<IWorkspaceInputCertification>();
        _changeDetector
            .Setup(item => item.BeginCertification(It.IsAny<string>()))
            .Returns(_inputCertification.Object);
        ...
        _target = new WorkspaceLifecycleService(
            ...
            _readOnlyDocumentValidator.Object,
    ...
    public async Task GIVEN_CancelledToken_WHEN_OpeningWorkspace_THEN_ShouldPropagateCancellation()
    {
        ...
        var action = async () => await _target.OpenAsync("Path", null, null, null, cancellationSource.Token);
    ...
    [Fact]
    public async Task GIVEN_InvalidMsBuildProperties_WHEN_OpeningWorkspace_THEN_ShouldReturnRejectionBeforeLoading()
    {
        ...
        var expected = CreateResult<WorkspaceOpenOutcome>();
        SetupOpenNormalization("/workspace/Project.csproj", alias: null);
        _msBuildPropertiesResolver.Setup(item => item.Resolve(properties))
            .Returns(WorkspaceMsBuildPropertiesResolution.Failure(error));
        SetupRejectedErrorResult(expected, "WorkspaceMsBuildPropertiesInvalid");
        ...
        result.Should().BeSameAs(expected);
        _workspaceLoadWorkflow.Verify(item => item.LoadAsync(
            It.IsAny<string>(),
            It.IsAny<string>(),
        ...
        string? existingAlias)
    {
        ...
        var result = await _target.OpenAsync("Path", alias, null, null, TestContext.Current.CancellationToken);
    ...
    [Fact]
    public async Task GIVEN_UnnormalisablePendingRecoveryPath_WHEN_OpeningWorkspace_THEN_ShouldRequireRecovery()
    {
        var solutionPath = Path.GetFullPath("/workspace/New.sln");
        var recoveryPath = "/workspace/Invalid.sln";
        var expected = CreateResult<WorkspaceOpenOutcome>();
        SetupOpenPreflight(solutionPath, alias: null);
        _recoveryStore.Setup(item => item.GetStatusesAsync(TestContext.Current.CancellationToken)).ReturnsAsync([
            new RecoveryStatus
        ...
        _workspacePathNormalizer
            .Setup(item => item.TryGetFullPath(recoveryPath, out It.Ref<string>.IsAny))
    ...
    [Fact]
    public async Task GIVEN_PendingRecoveryForDifferentSolutionWithSharedRoot_WHEN_OpeningWorkspace_THEN_ShouldContinueToLoad()
    {
        var expected = CreateResult<WorkspaceOpenOutcome>();
        SetupOpenPreflight("/workspace/New.sln", alias: null);
        _recoveryStore.Setup(item => item.GetStatusesAsync(TestContext.Current.CancellationToken)).ReturnsAsync([
    ...
    [Fact]
    public async Task GIVEN_PendingRecoveryForDifferentSolutionWithMalformedRoot_WHEN_OpeningWorkspace_THEN_ShouldRequireRecovery()
    {
        var recoveryRoot = "InvalidRoot";
        var expected = CreateResult<WorkspaceOpenOutcome>();
        SetupOpenPreflight("/workspace/New.sln", alias: null);
        _recoveryStore.Setup(item => item.GetStatusesAsync(TestContext.Current.CancellationToken)).ReturnsAsync([
            new RecoveryStatus
        ...
        _workspacePathNormalizer
            .Setup(item => item.TryGetFullPath(recoveryRoot, out It.Ref<string>.IsAny))
            .Returns((string _, out string normalizedPath) =>
            {
                normalizedPath = string.Empty;
                return false;
    ...
    public async Task GIVEN_ValidWorkspace_WHEN_OpeningWorkspace_THEN_ShouldStoreAndReturnWorkspace()
    {
        ...
        _msBuildPropertiesResolver.Setup(item => item.Resolve(requestedProperties))
            .Returns(WorkspaceMsBuildPropertiesResolution.Success(resolvedProperties));
        loadedWorkspace.SetupGet(item => item.CurrentSolution).Returns(solution);
        _workspaceLoadWorkflow.Setup(item => item.LoadAsync(
            "/workspace/New.sln",
            "/workspace",
            resolvedProperties,
            TestContext.Current.CancellationToken))
        ...
        _resultFactory.Setup(item => item.Succeeded(
            It.Is<WorkspaceOpenOutcome>(outcome =>
                outcome.Workspace.WorkspaceId == Guid.Parse("11111111-1111-1111-1111-111111111111")
                && outcome.ProjectCount == 1),
    ...
    [Fact]
    public async Task GIVEN_WorkspaceInputsChangeDuringLoad_WHEN_OpeningWorkspace_THEN_ShouldDisposeAndRequireRetry()
    {
        var loadedWorkspace = new Mock<ILoadedWorkspace>();
        var solution = CreateSolutionWithProject("/workspace/Project.csproj");
        ...
        _resultFactory.Setup(item => item.Rejected<WorkspaceOpenOutcome>(
        ...
        _instanceStatusPublisher.Verify(
            item => item.CloseAsync(Guid.Parse("11111111-1111-1111-1111-111111111111")),
            Times.Once);
    }

    [Fact]
    public async Task GIVEN_ExternalDocumentChangesDuringLoad_WHEN_OpeningWorkspace_THEN_ShouldDisposeAndRequireRetry()
    {
        var loadedWorkspace = new Mock<ILoadedWorkspace>();
        var solution = CreateSolutionWithProject("/workspace/Project.csproj");
        using var manifest = new WorkspaceInputManifest();
        ...
        _readOnlyDocumentValidator
            .Setup(item => item.ValidateAsync(solution, "/workspace", TestContext.Current.CancellationToken))
            .ReturnsAsync(WorkspaceReadOnlyDocumentValidationStatus.Invalid);
    ...
    [Fact]
    public async Task GIVEN_SessionCleanupFails_WHEN_Closing_THEN_ShouldPropagateFailureWithRemovedWorkspaceContext()
    {
        var operationLease = new Mock<IWorkspaceOperationLease>();
        var gate = new Mock<IWorkspaceOperationGate>();
        var session = CreateSession(Guid.Parse("11111111-1111-1111-1111-111111111111"), "Path", alias: null, transaction: null) with { OperationGate = gate.Object };
        ...
        var assertion = await action.Should().ThrowAsync<WorkspaceOperationException>();
        assertion.Which.InnerException.Should().BeSameAs(failure);
        assertion.Which.Context.Workspace.WorkspaceId.Should().Be(session.Workspace.WorkspaceId);
        assertion.Which.Context.Workspace.WorkspaceEpoch.Should().Be(session.Workspace.WorkspaceEpoch);
        assertion.Which.Context.LifecycleState.Should().Be(WorkspaceLifecycleState.Ready);
        assertion.Which.Context.ProjectCount.Should().Be(session.ProjectCount);
        assertion.Which.Context.DocumentCount.Should().Be(session.DocumentCount);
    }
    ...
    public async Task GIVEN_OutOfDateWorkspace_WHEN_Reloading_THEN_ShouldReplaceSessionAndDisposeOldWorkspace()
    {
        ...
        var expected = CreateResult<WorkspaceReloadOutcome>();
        SetupSelectedSession(session, gate, operationLease, exclusive: true);
        newWorkspace.SetupGet(item => item.CurrentSolution).Returns(solution);
        _workspaceLoadWorkflow.Setup(item => item.LoadAsync(
            solutionPath,
            workspaceRoot,
            properties,
        ...
        _resultFactory.Setup(item => item.Succeeded(
            It.Is<WorkspaceReloadOutcome>(outcome =>
                outcome.Workspace.WorkspaceId == Guid.Parse("11111111-1111-1111-1111-111111111111")
                && outcome.Workspace.WorkspaceEpoch == 3),
            It.IsAny<WorkspaceOperationContext>(),
            null,
            null)).Returns(expected);
    ...
    [Fact]
    public async Task GIVEN_ReplacedWorkspaceDisposalFails_WHEN_Reloading_THEN_ShouldPropagateFailureWithReplacedWorkspaceContext()
    {
        var solutionPath = Path.GetFullPath("/workspace/Solution.sln");
        var workspaceRoot = Path.GetDirectoryName(solutionPath)!;
        var operationLease = new Mock<IWorkspaceOperationLease>();
        ...
        _sessionStore.Setup(item => item.AllocateWorkspaceEpoch()).Returns(3);
        oldWorkspace.Setup(item => item.Dispose()).Throws(failure);

        var action = async () => await _target.ReloadAsync(null, null, null, TestContext.Current.CancellationToken);

        var assertion = await action.Should().ThrowAsync<WorkspaceOperationException>();
        assertion.Which.InnerException.Should().BeSameAs(failure);
        assertion.Which.Context.Workspace.WorkspaceId.Should().Be(session.Workspace.WorkspaceId);
        assertion.Which.Context.Workspace.WorkspaceEpoch.Should().Be(session.Workspace.WorkspaceEpoch);
        assertion.Which.Context.LifecycleState.Should().Be(WorkspaceLifecycleState.WorkspaceOutOfDate);
    ...
    [Fact]
    public async Task GIVEN_ReloadedStatusPublicationFails_WHEN_Reloading_THEN_ShouldPropagateFailureWithReloadedWorkspaceContext()
    {
        var solutionPath = Path.GetFullPath("/workspace/Solution.sln");
        var workspaceRoot = Path.GetDirectoryName(solutionPath)!;
        var operationLease = new Mock<IWorkspaceOperationLease>();
        ...
        assertion.Which.InnerException.Should().BeSameAs(failure);
        assertion.Which.Context.Workspace.WorkspaceId.Should().Be(session.Workspace.WorkspaceId);
        assertion.Which.Context.Workspace.WorkspaceEpoch.Should().Be(3);
        assertion.Which.Context.LifecycleState.Should().Be(WorkspaceLifecycleState.Ready);
        assertion.Which.Context.ProjectCount.Should().Be(_workspace.CurrentSolution.ProjectIds.Count);
        _sessionStore.Verify(item => item.ReplaceSession(It.IsAny<WorkspaceSessionSnapshot>()), Times.Once);
        oldWorkspace.Verify(item => item.Dispose(), Times.Once);
    }
    ...
    [Fact]
    public async Task GIVEN_ExternalDocumentChangesDuringLoad_WHEN_ReloadingWorkspace_THEN_ShouldRetainOldSessionAndRequireRetry()
    {
        var solutionPath = Path.GetFullPath("/workspace/Solution.sln");
        var workspaceRoot = Path.GetDirectoryName(solutionPath)!;
        var projectPath = Path.Combine(workspaceRoot, "Project.csproj");
