...
| 5 | In progress | Protocol schema pipeline | RWMCP-028 (completed) → RWMCP-021 | Full output schemas now match supported success and no-change payloads. Preflight the same authoritative request and configured-response schemas during plugin admission so one unsupported contract disables only its owning plugin before DI publication. |
