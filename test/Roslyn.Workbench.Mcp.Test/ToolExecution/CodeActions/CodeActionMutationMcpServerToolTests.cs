...
public sealed class CodeActionMutationMcpServerToolTests
{
    ...
    public async Task GIVEN_ContextAcquisitionFailure_WHEN_InvokingMutation_THEN_ShouldPublishFailureWithoutCallingHandlerAndDisposeLease()
    {
        ...
        contextFactory
            ...
            .Returns(new CodeActionMutationExecutionLease(workspaceLease, context: null, failure));
    ...
    [Fact]
    public async Task GIVEN_AcquiredLeaseWithoutContext_WHEN_InvokingMutation_THEN_ShouldPublishUnhandledFailureAndDisposeLease()
    {
        var handler = new Mock<ICodeActionMutationToolHandler<TestMutationRequest>>();
        var contextFactory = new Mock<ICodeActionExecutionContextFactory>();
        ...
        contextFactory
            ...
            .Returns(new CodeActionMutationExecutionLease(workspaceLease, context: null, failure: null));
        var target = CreateTarget(handler.Object, contextFactory.Object);
        ...
        string outcomeName)
    {
        ...
        contextFactory
            .Setup(item => item.CreateMutationContext(It.IsAny<WorkspaceBoundRequest>(), CancellationToken.None))
            .Returns(new CodeActionMutationExecutionLease(workspaceLease, context.Object, failure: null));
        handler
            .Setup(item => item.ExecuteAsync(It.IsAny<TestMutationRequest>(), context.Object, CancellationToken.None))
            .ReturnsAsync(new CodeActionExecutionResult<WorkspaceMutationProposal>
            {
                Outcome = outcome,
    ...
    public async Task GIVEN_HandlerNoChange_WHEN_InvokingMutation_THEN_ShouldPublishUnstagedSuccess()
    {
        ...
        handler
            .Setup(item => item.ExecuteAsync(It.IsAny<TestMutationRequest>(), context.Object, CancellationToken.None))
            .ReturnsAsync(CodeActionExecutionResult<WorkspaceMutationProposal>.NoChange());
        var target = CreateTarget(handler.Object, contextFactory.Object);
    ...
    public async Task GIVEN_HandlerProposalAndSuccessfulStaging_WHEN_InvokingMutation_THEN_ShouldStageProposalAndPublishSuccess()
    {
        ...
        var proposal = new WorkspaceMutationProposal
        {
            Summary = "Summary",
        };
        var workspaceLease = WorkspaceMutationExecutionLease.Acquired(
        ...
        handler
            .Setup(item => item.ExecuteAsync(It.IsAny<TestMutationRequest>(), context.Object, CancellationToken.None))
            .ReturnsAsync(CodeActionExecutionResult<WorkspaceMutationProposal>.Success(
                proposal,
    ...
    public async Task GIVEN_HandlerProposalAndRejectedStaging_WHEN_InvokingMutation_THEN_ShouldPublishStagingFailure()
    {
        ...
        handler
            .Setup(item => item.ExecuteAsync(It.IsAny<TestMutationRequest>(), context.Object, CancellationToken.None))
            .ReturnsAsync(CodeActionExecutionResult<WorkspaceMutationProposal>.Success(new WorkspaceMutationProposal
            {
                Summary = "Summary",
            }));
        stager
    ...
    public async Task GIVEN_HandlerThrows_WHEN_InvokingMutation_THEN_ShouldPublishUnhandledFailureAndDisposeLease()
    {
        ...
        handler
            .Setup(item => item.ExecuteAsync(It.IsAny<TestMutationRequest>(), context.Object, CancellationToken.None))
            .Returns(ValueTask.FromException<CodeActionExecutionResult<WorkspaceMutationProposal>>(new InvalidOperationException("Message")));
        var target = CreateTarget(handler.Object, contextFactory.Object);
    ...
    public async Task GIVEN_HandlerCancellation_WHEN_InvokingMutation_THEN_ShouldPropagateCancellationAndDisposeLease()
    {
        ...
        handler
            ...
            .Returns(ValueTask.FromCanceled<CodeActionExecutionResult<WorkspaceMutationProposal>>(cancellationSource.Token));
        var target = CreateTarget(handler.Object, contextFactory.Object);
