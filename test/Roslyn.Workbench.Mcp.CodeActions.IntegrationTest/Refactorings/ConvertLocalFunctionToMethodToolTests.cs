...
public sealed class ConvertLocalFunctionToMethodToolIntegrationTests
{
    [Fact]
    public async Task GIVEN_ActiveTransaction_WHEN_ExecutingTool_THEN_ShouldStageMutation()
    {
        ...
        var result = await BundledCoreToolTestHarness.ExecuteMutationAsync(coordinator, "convert-local-function-to-method", target, RefactoringRequestFactory.CreateLocationRequest(fixture.GetLocation("Local", 0), openResult));
        ...
        result.StagedResult.Data!.Operation.Should().Be("convert-local-function-to-method");
