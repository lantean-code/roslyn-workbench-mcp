...
internal sealed class CodeActionDiscoveryService : ICodeActionDiscoveryService
{
    ...
    {
        return _codeFixProviders.SingleOrDefault(candidate => string.Equals(GetProviderId(candidate), providerId, StringComparison.Ordinal));
    }
    ...
    {
        var provider = _refactoringProviders.SingleOrDefault(candidate => string.Equals(GetProviderId(candidate), providerId, StringComparison.Ordinal));
    ...
    public async ValueTask<IReadOnlyList<DiscoveredCodeAction>> DiscoverProviderCodeFixesAsync(
        ...
        CancellationToken cancellationToken)
    {
        var provider = _codeFixProviders.SingleOrDefault(candidate => string.Equals(GetProviderId(candidate), providerId, StringComparison.Ordinal));
        return provider is null
            ? []
