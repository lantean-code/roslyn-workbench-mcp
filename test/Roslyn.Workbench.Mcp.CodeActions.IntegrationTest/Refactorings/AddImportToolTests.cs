...
public sealed class AddImportToolIntegrationTests
{
    [Fact]
    public async Task GIVEN_ActiveTransaction_WHEN_ExecutingTool_THEN_ShouldStageMutation()
    {
        ...
        var result = await BundledCoreToolTestHarness.ExecuteMutationAsync(coordinator, "add-import", target, RefactoringRequestFactory.CreateAddImportRequest(fixture.GetCursor("System.Net.Http.HttpClient"), openResult, simplifyAllOccurrences: false));
        ...
        result.StagedResult.Data!.Operation.Should().Be("add-import");
