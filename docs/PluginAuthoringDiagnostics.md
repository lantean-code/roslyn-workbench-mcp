...
Apply a curated limit before constructing the response and publish the result through `BoundedCollection.CreatePrebounded<TItem>` from `Roslyn.Workbench.Mcp.Workspace.Results`. Include `TotalCount` only when the complete count is already available cheaply.
