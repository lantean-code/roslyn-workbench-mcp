...
public sealed class ConvertForToForeachToolIntegrationTests
{
    [Fact]
    public async Task GIVEN_ActiveTransaction_WHEN_ExecutingTool_THEN_ShouldStageMutation()
    {
        ...
        var result = await BundledCoreToolTestHarness.ExecuteMutationAsync(coordinator, "convert-for-to-foreach", target, RefactoringRequestFactory.CreateLocationRequest(fixture.GetLocation("for (var i = 0; i < values.Length; i++)", 0), openResult));
        ...
        result.StagedResult.Data!.Operation.Should().Be("convert-for-to-foreach");
