...
public sealed class WorkspaceTransactionIntegrationTests
{
    ...
    [Fact]
    public async Task GIVEN_GeneratedLookingCheckedInSourceAndWarnPolicy_WHEN_StagingAndPreviewing_THEN_ShouldReportWarning()
    {
        using var fixture = TestWorkspaceFixture.Create();
        var existingSource = await File.ReadAllTextAsync(fixture.DocumentPath, TestContext.Current.CancellationToken);
        ...
        var stage = await StageMutationAsync(target);
        var preview = await target.PreviewTransactionAsync(TestContext.Current.CancellationToken);

        stage.Outcome.Should().Be(PluginExecutionOutcome.Succeeded);
        stage.Warnings.Should().ContainSingle(warning => warning.Code == GeneratedSourceMutationWarnings.WarningCode);
        preview.Warnings.Should().ContainSingle(warning => warning.Code == GeneratedSourceMutationWarnings.WarningCode);
    }

    [Fact]
    public async Task GIVEN_GeneratedLookingCheckedInSourceAndDenyPolicy_WHEN_Staging_THEN_ShouldRejectMutation()
    {
        using var fixture = TestWorkspaceFixture.Create();
        var existingSource = await File.ReadAllTextAsync(fixture.DocumentPath, TestContext.Current.CancellationToken);
        await File.WriteAllTextAsync(
        ...
        var options = new ComponentWorkspaceOptions { GeneratedSourcePolicy = "deny" };
        await using var target = fixture.CreateWorkspace(options);
        await target.OpenAsync(fixture.ProjectPath, TestContext.Current.CancellationToken);
        await target.StartTransactionAsync(TestContext.Current.CancellationToken);

        var stage = await StageMutationAsync(target);

        stage.Outcome.Should().Be(PluginExecutionOutcome.Rejected);
        stage.Error?.Code.Should().Be(WorkspaceErrorCodes.GeneratedSourceMutationDenied);
    }
