...
- [x] Replace sequential source writes with a durable multi-file write-ahead commit protocol. Notes:
...
- [x] Remove remaining plugin result leakage from workspace internals. Files:
...
- [x] Replace manual plugin activation and loose-DLL discovery with validated MEF package composition. Files:
  - `src/Roslyn.Workbench.Mcp/PluginCatalogLoader.cs` Notes:
...
- [x] Reduce null-forgiving and weak invariants in first-party plugin tools. Files:
  ...
  - `src/Roslyn.Workbench.Mcp.Plugins.Core/ReplayCodeActionExecutor.cs` Notes:
...
- [x] Reduce null-forgiving and invariant-by-convention in workspace support code. Files:
  ...
  - `src/Roslyn.Workbench.Mcp.Workspace/WorkspaceTransactionRevision.cs` Notes:
...
- [x] Tighten remaining invariant bridges in server-host mapping code. Files:
  - `src/Roslyn.Workbench.Mcp/WorkspaceToolResultMapper.cs` Notes:
...
- [x] `Roslyn.Workbench.Mcp.Plugins` Notes:
...
- [x] `Roslyn.Workbench.Mcp.Plugins.Core` Notes:
  - Bundled tools use the shared MEF plugin path and the earlier nullability concerns have been remediated.

## Suggested Working Order

The recommended order is now explicitly workspace-first. The plugin and host work should follow after the central workspace boundaries are in a better state.
