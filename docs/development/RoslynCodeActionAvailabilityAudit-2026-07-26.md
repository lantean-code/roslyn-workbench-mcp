...
The composed runtime contains 101 visible providers and 149 hidden providers. The hidden providers now divide as follows:

| Outcome | Count | Release interpretation |
| --- | ---: | --- |
| Replay candidate | 25 | The remaining compiler-backed code fixes require compatibility validation rather than a new transformation. |
...
Forty-seven providers consume compiler diagnostics already available from `Compilation.GetDiagnostics`. Twenty-two providers have passed deterministic action selection and transaction-staging validation and are published through twenty-one dedicated tools. The remaining 25 are tracked as `PendingReplayValidation` only by the development audit. Compiler diagnostic availability establishes reachability but does not by itself prove deterministic action selection or staging.
...
Validation completed with 529 Code Actions unit and contract tests, 153 runtime-provider audit tests, 53 Host integration tests and 50 published-host acceptance tests. The acceptance suite includes a catalogue-locked matrix for all 70 dedicated Code Action tools and 81 independent execution paths. It verifies that every tool is published, then independently stages, previews and rolls back a successful invocation against the controlled workspace while proving that changed files are restored and newly created files are absent after rollback. A separate published-host workflow lists two otherwise-identical `CS8603` fixes from one broad range, verifies their distinct resolved locations and stages each correct mutation independently using only its opaque GUID. The complete GuardClauses measurement suite also passed against the expanded catalogue, including Code Action discovery, formatting, `organize-imports`, rename and representative query scenarios; validation confirmed normal Host shutdown, the pinned commit, no tracked repository changes and no leaked recovery or Workspace state. Latest evidence: `artifacts/performance/results/20260727-172352-guardclauses-65ed631832a346a591fe826464c291de`.
...
`CandidateCompatibilityCases` permanently tracks the 25 compiler-backed code-fix candidates that are not yet published. Twenty-two providers live in the supported compatibility suite and are published through twenty-one dedicated tools: anonymous-member naming, explicit casts, `inheritdoc`, obsolete attributes, both unassigned-`out`-parameter placements, nullable declarations, incorrect constraints, return-type repair, unnecessary `new`, conditional interpolation parentheses, invalid `in` arguments, default-literal replacement, explicit const types, intentional base-member hiding, both add-yield diagnostics, iterator return-type repair, required members, record-keyword placement, compiler-driven modifier ordering, unused local-function removal and explicit expression-tree arrays. Each advertised diagnostic route passes the real pinned MEF composition, expected compiler diagnostic, deterministic action selection, supported `ApplyChangesOperation`, expected source mutation, dedicated handler, transaction staging and preview path.
...
The retrospective check is complete for `add-obsolete-attribute`, `declare-as-nullable`, `fix-incorrect-constraint` and `fix-return-type`. Their fourteen advertised diagnostic IDs each produce exactly one replayable action at a precise diagnostic location and each succeeds through published-host staging, preview and rollback. The promoted `add-yield` tool has the same evidence for both `CS0029` and `CS0266`, bringing the diagnostic-specific total for multi-diagnostic tools to sixteen. The diagnostic routes can target different declarations, but the existing `LocationSelector` already provides the necessary disambiguation; adding a separate diagnostic-ID request property would be redundant and would still not distinguish multiple occurrences of the same ID. Dedicated location-targeted tools continue to reject broad locations containing distinct candidates as ambiguous. Broad `list-code-actions` queries instead publish one precise resolved location and opaque replay reference per candidate, allowing the caller to choose and stage the intended occurrence by GUID.
